...
1. 從 [ **手動和排程的組建** ] 下拉式清單中，選取預設分支中的 *主要* 分支。 按一下 [繼續] 按鈕。 [範本選取] 頁面隨即出現。
...
1. 勾選 [ **啟用持續整合** ] 方塊。 在 [ **分支篩選** ] 區段下，確認 [ **類型** ] 下拉式清單已設定為 [ *包含* ]。 將 [ **分支規格** ] 下拉式清單設定為 [ *master* ]。
    ...
    這些設定會在任何變更推送至 GitHub 存放庫的 *主要* 分支時，造成組建觸發程式。 持續整合會在將 [變更認可至 GitHub 並自動部署至 Azure](#commit-changes-to-github-and-automatically-deploy-to-azure) 區段中進行測試。
...
1. 在 [ **組建分支篩選** ] 區段中，按一下 [ **新增** ] 下拉式清單。 選擇 [ **組建定義** ] 的 [預設分支] 選項。 此篩選器只會對 GitHub 存放庫的 *主要* 分支中的組建觸發發行。
...
1. 將 *master* 分支中的變更推送至 GitHub 存放庫的 *原始* 遠端：
