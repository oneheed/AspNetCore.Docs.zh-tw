...
---
# <a name="aspnet-core-no-locblazor-logging"></a><span data-ttu-id="ff317-103">ASP.NET Core Blazor 記錄</span><span class="sxs-lookup"><span data-stu-id="ff317-103">ASP.NET Core Blazor logging</span></span>

## Blazor WebAssembly

<span data-ttu-id="ff317-104">Blazor WebAssembly使用中的屬性來設定應用程式中的記錄 `WebAssemblyHostBuilder.Logging` `Program.Main` ：</span><span class="sxs-lookup"><span data-stu-id="ff317-104">Configure logging in Blazor WebAssembly apps with the `WebAssemblyHostBuilder.Logging` property in `Program.Main`:</span></span>
...
## <a name="log-in-no-locrazor-components"></a><span data-ttu-id="ff317-113">登入 Razor 元件</span><span class="sxs-lookup"><span data-stu-id="ff317-113">Log in Razor components</span></span>
...
<span data-ttu-id="ff317-116">下列範例示範如何使用 <xref:Microsoft.Extensions.Logging.ILogger> in 元件進行記錄 Razor ：</span><span class="sxs-lookup"><span data-stu-id="ff317-116">The following example demonstrates logging with an <xref:Microsoft.Extensions.Logging.ILogger> in Razor components:</span></span>
...
<span data-ttu-id="ff317-117">下列範例示範如何使用 <xref:Microsoft.Extensions.Logging.ILoggerFactory> in 元件進行記錄 Razor ：</span><span class="sxs-lookup"><span data-stu-id="ff317-117">The following example demonstrates logging with an <xref:Microsoft.Extensions.Logging.ILoggerFactory> in Razor components:</span></span>
