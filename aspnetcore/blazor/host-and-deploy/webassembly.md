...
## <a name="compression"></a><span data-ttu-id="b8d8e-112">壓縮</span><span class="sxs-lookup"><span data-stu-id="b8d8e-112">Compression</span></span>
...
* <span data-ttu-id="b8d8e-121">裝載在不支援靜態壓縮的檔案內容協商的靜態裝載方案（例如 GitHub 頁面）時，請考慮將應用程式設定為提取和解碼 Brotli 壓縮檔案：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-121">When hosting on static hosting solutions that don't support statically-compressed file content negotiation, such as GitHub Pages, consider configuring the app to fetch and decode Brotli compressed files:</span></span>
  ...
  * <span data-ttu-id="b8d8e-127">更新應用程式以使用此解碼器。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-127">Update the app to use the decoder.</span></span> <span data-ttu-id="b8d8e-128">將結束記號內的標記變更 `<body>` `wwwroot/index.html` 為下列內容：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-128">Change the markup inside the closing `<body>` tag in `wwwroot/index.html` to the following:</span></span>
...
## <a name="hosted-deployment-with-aspnet-core"></a><span data-ttu-id="b8d8e-150">搭配 ASP.NET Core 的已裝載部署</span><span class="sxs-lookup"><span data-stu-id="b8d8e-150">Hosted deployment with ASP.NET Core</span></span>
...
<span data-ttu-id="b8d8e-163">在下例中︰</span><span class="sxs-lookup"><span data-stu-id="b8d8e-163">In the following example:</span></span>
...
<span data-ttu-id="b8d8e-202">針對靜態資產，請使用下列方法：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-202">Use the following approaches for static assets:</span></span>
...
<span data-ttu-id="b8d8e-234">Blazor發行專案時， `web.config` 會使用下列 IIS 設定來建立檔案：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-234">When a Blazor project is published, a `web.config` file is created with the following IIS configuration:</span></span>
...
* <span data-ttu-id="b8d8e-267">停用繼承的 ASP.NET Core 模組處理常式。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-267">Disable the inherited ASP.NET Core Module handler.</span></span>
  ...
  <span data-ttu-id="b8d8e-268">Blazor `web.config` 將區段新增至檔案，以移除應用程式已發佈檔案中的處理常式 `<handlers>` ：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-268">Remove the handler in the Blazor app's published `web.config` file by adding a `<handlers>` section to the file:</span></span>
...
<span data-ttu-id="b8d8e-276">在下列情況下，可能需要額外的範例檔案設定 `web.config` ：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-276">Additional configuration of the example `web.config` file might be required in the following scenarios:</span></span>
...
<span data-ttu-id="b8d8e-296">如果檔案的標頭中有不適當的 MIME 類型，則不會在執行時間載入檔案 `Content-Type` ，請採取下列其中一項動作：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-296">If files aren't loaded at runtime due to inappropriate MIME types in the files' `Content-Type` headers, take either of the following actions:</span></span>
...
### <a name="urls"></a><span data-ttu-id="b8d8e-348">URL</span><span class="sxs-lookup"><span data-stu-id="b8d8e-348">URLs</span></span>
...
## <a name="configure-the-linker"></a><span data-ttu-id="b8d8e-359">設定連結器</span><span class="sxs-lookup"><span data-stu-id="b8d8e-359">Configure the Linker</span></span>
...
* <span data-ttu-id="b8d8e-367">將每個要求重新導向至新名稱，將別名資源重新導向至不同的名稱 `fetch` 。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-367">Alias resources to a different name by redirecting each `fetch` request to a new name.</span></span>

<span data-ttu-id="b8d8e-368">`loadBootResource` 參數會出現在下表中。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-368">`loadBootResource` parameters appear in the following table.</span></span>

| <span data-ttu-id="b8d8e-369">參數</span><span class="sxs-lookup"><span data-stu-id="b8d8e-369">Parameter</span></span>    | <span data-ttu-id="b8d8e-370">Description</span><span class="sxs-lookup"><span data-stu-id="b8d8e-370">Description</span></span> |
| ------------ | ----------- |
| `type`       | <span data-ttu-id="b8d8e-371">資源類型。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-371">The type of the resource.</span></span> <span data-ttu-id="b8d8e-372">運算子類型： `assembly` 、 `pdb` 、 `dotnetjs` 、 `dotnetwasm` 、 `timezonedata`</span><span class="sxs-lookup"><span data-stu-id="b8d8e-372">Permissable types: `assembly`, `pdb`, `dotnetjs`, `dotnetwasm`, `timezonedata`</span></span> |
| `name`       | <span data-ttu-id="b8d8e-373">資源名稱。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-373">The name of the resource.</span></span> |
| `defaultUri` | <span data-ttu-id="b8d8e-374">資源的相對或絕對 URI。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-374">The relative or absolute URI of the resource.</span></span> |
| `integrity`  | <span data-ttu-id="b8d8e-375">代表回應中預期內容的完整性字串。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-375">The integrity string representing the expected content in the response.</span></span> |

<span data-ttu-id="b8d8e-376">`loadBootResource` 傳回下列任一項以覆寫載入進程：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-376">`loadBootResource` returns any of the following to override the loading process:</span></span>

* <span data-ttu-id="b8d8e-377">URI 字串。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-377">URI string.</span></span> <span data-ttu-id="b8d8e-378">在下列範例 (`wwwroot/index.html`) 中，從 CDN 提供下列檔案 `https://my-awesome-cdn.com/` ：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-378">In the following example (`wwwroot/index.html`), the following files are served from a CDN at `https://my-awesome-cdn.com/`:</span></span>

  * `dotnet.*.js`
  * `dotnet.wasm`
  * <span data-ttu-id="b8d8e-379">時區資料</span><span class="sxs-lookup"><span data-stu-id="b8d8e-379">Timezone data</span></span>
...
* <span data-ttu-id="b8d8e-380">`Promise<Response>`.</span><span class="sxs-lookup"><span data-stu-id="b8d8e-380">`Promise<Response>`.</span></span> <span data-ttu-id="b8d8e-381">`integrity`在標頭中傳遞參數，以保留預設的完整性檢查行為。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-381">Pass the `integrity` parameter in a header to retain the default integrity-checking behavior.</span></span>

  <span data-ttu-id="b8d8e-382">下列範例 (`wwwroot/index.html`) 將自訂 HTTP 標頭新增至輸出要求，並將 `integrity` 參數傳遞至 `fetch` 呼叫：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-382">The following example (`wwwroot/index.html`) adds a custom HTTP header to the outbound requests and passes the `integrity` parameter through to the `fetch` call:</span></span>
...
* <span data-ttu-id="b8d8e-383">`null`/`undefined`，這會導致預設的載入行為。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-383">`null`/`undefined`, which results in the default loading behavior.</span></span>

<span data-ttu-id="b8d8e-384">外部來源必須傳回瀏覽器所需的 CORS 標頭，以允許跨原始來源資源載入。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-384">External sources must return the required CORS headers for browsers to allow the cross-origin resource loading.</span></span> <span data-ttu-id="b8d8e-385">依預設，Cdn 通常會提供必要的標頭。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-385">CDNs usually provide the required headers by default.</span></span>

<span data-ttu-id="b8d8e-386">您只需要指定自訂行為的類型。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-386">You only need to specify types for custom behaviors.</span></span> <span data-ttu-id="b8d8e-387">未指定的類型 `loadBootResource` 會根據其預設載入行為載入架構。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-387">Types not specified to `loadBootResource` are loaded by the framework per their default loading behaviors.</span></span>

## <a name="change-the-filename-extension-of-dll-files"></a><span data-ttu-id="b8d8e-388">變更 DLL 檔案的副檔名</span><span class="sxs-lookup"><span data-stu-id="b8d8e-388">Change the filename extension of DLL files</span></span>

<span data-ttu-id="b8d8e-389">如果您需要變更應用程式已發佈檔案的副檔名 `.dll` ，請遵循本節中的指導方針。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-389">In case you have a need to change the filename extensions of the app's published `.dll` files, follow the guidance in this section.</span></span>

<span data-ttu-id="b8d8e-390">發佈應用程式之後，請使用 shell 腳本或 DevOps 組建管線，將檔案重新命名 `.dll` 以使用不同的副檔名。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-390">After publishing the app, use a shell script or DevOps build pipeline to rename `.dll` files to use a different file extension.</span></span> <span data-ttu-id="b8d8e-391">以 `.dll` `wwwroot` 應用程式發佈輸出的目錄中的檔案為目標 (例如 `{CONTENT ROOT}/bin/Release/netstandard2.1/publish/wwwroot`) 。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-391">Target the `.dll` files in the `wwwroot` directory of the app's published output (for example, `{CONTENT ROOT}/bin/Release/netstandard2.1/publish/wwwroot`).</span></span>

<span data-ttu-id="b8d8e-392">在下列範例中， `.dll` 會將檔案重新命名為使用 `.bin` 副檔名。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-392">In the following examples, `.dll` files are renamed to use the `.bin` file extension.</span></span>

<span data-ttu-id="b8d8e-393">在 Windows 上：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-393">On Windows:</span></span>
...
<span data-ttu-id="b8d8e-394">如果服務工作者資產也在使用中，請新增下列命令：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-394">If service worker assets are also in use, add the following command:</span></span>

```powershell
((Get-Content .\service-worker-assets.js -Raw) -replace '.dll"','.bin"') | Set-Content .\service-worker-assets.js
```

<span data-ttu-id="b8d8e-395">在 Linux 或 macOS 上：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-395">On Linux or macOS:</span></span>
...
<span data-ttu-id="b8d8e-396">如果服務工作者資產也在使用中，請新增下列命令：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-396">If service worker assets are also in use, add the following command:</span></span>

```console
sed -i 's/\.dll"/.bin"/g' service-worker-assets.js
```
   
<span data-ttu-id="b8d8e-397">若要使用不同的副檔名 `.bin` ，請 `.bin` 在上述命令中取代。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-397">To use a different file extension than `.bin`, replace `.bin` in the preceding commands.</span></span>

<span data-ttu-id="b8d8e-398">若要處理壓縮 `blazor.boot.json.gz` 和檔案 `blazor.boot.json.br` ，請採用下列其中一種方法：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-398">To address the compressed `blazor.boot.json.gz` and `blazor.boot.json.br` files, adopt either of the following approaches:</span></span>

* <span data-ttu-id="b8d8e-399">移除壓縮的 `blazor.boot.json.gz` 和檔案 `blazor.boot.json.br` 。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-399">Remove the compressed `blazor.boot.json.gz` and `blazor.boot.json.br` files.</span></span> <span data-ttu-id="b8d8e-400">這種方法會停用壓縮。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-400">Compression is disabled with this approach.</span></span>
* <span data-ttu-id="b8d8e-401">重新壓縮更新後的檔案 `blazor.boot.json` 。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-401">Recompress the updated `blazor.boot.json` file.</span></span>

<span data-ttu-id="b8d8e-402">先前的指導方針也適用于服務工作者資產正在使用中。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-402">The preceding guidance also applies when service worker assets are in use.</span></span> <span data-ttu-id="b8d8e-403">移除或重新壓縮 `wwwroot/service-worker-assets.js.br` 和 `wwwroot/service-worker-assets.js.gz` 。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-403">Remove or recompress `wwwroot/service-worker-assets.js.br` and `wwwroot/service-worker-assets.js.gz`.</span></span> <span data-ttu-id="b8d8e-404">否則，瀏覽器中的檔案完整性檢查會失敗。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-404">Otherwise, file integrity checks fail in the browser.</span></span>

<span data-ttu-id="b8d8e-405">下列 Windows 範例使用放置於專案根目錄的 PowerShell 腳本。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-405">The following Windows example uses a PowerShell script placed at the root of the project.</span></span>

<span data-ttu-id="b8d8e-406">`ChangeDLLExtensions.ps1:`:</span><span class="sxs-lookup"><span data-stu-id="b8d8e-406">`ChangeDLLExtensions.ps1:`:</span></span>
...
<span data-ttu-id="b8d8e-407">如果服務工作者資產也在使用中，請新增下列命令：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-407">If service worker assets are also in use, add the following command:</span></span>

```powershell
((Get-Content $filepath\bin\Release\$tfm\wwwroot\service-worker-assets.js -Raw) -replace '.dll"','.bin"') | Set-Content $filepath\bin\Release\$tfm\wwwroot\service-worker-assets.js
```

<span data-ttu-id="b8d8e-408">在專案檔中，腳本會在發佈應用程式之後執行：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-408">In the project file, the script is run after publishing the app:</span></span>
...
> [!NOTE]
> <span data-ttu-id="b8d8e-409">重新命名和消極式載入相同的元件時，請參閱中的指導方針 <xref:blazor/webassembly-lazy-load-assemblies#onnavigateasync-events-and-renamed-assembly-files> 。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-409">When renaming and lazy loading the same assemblies, see the guidance in <xref:blazor/webassembly-lazy-load-assemblies#onnavigateasync-events-and-renamed-assembly-files>.</span></span>

## <a name="resolve-integrity-check-failures"></a><span data-ttu-id="b8d8e-410">解決完整性檢查失敗</span><span class="sxs-lookup"><span data-stu-id="b8d8e-410">Resolve integrity check failures</span></span>

<span data-ttu-id="b8d8e-411">當 Blazor WebAssembly 下載應用程式的啟動檔案時，它會指示瀏覽器對回應執行完整性檢查。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-411">When Blazor WebAssembly downloads an app's startup files, it instructs the browser to perform integrity checks on the responses.</span></span> <span data-ttu-id="b8d8e-412">它會使用檔案中的資訊 `blazor.boot.json` `.dll` ，為、和其他檔案指定預期的 256 sha-1 雜湊值 `.wasm` 。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-412">It uses information in the `blazor.boot.json` file to specify the expected SHA-256 hash values for `.dll`, `.wasm`, and other files.</span></span> <span data-ttu-id="b8d8e-413">這項功能很有用，原因如下：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-413">This is beneficial for the following reasons:</span></span>

* <span data-ttu-id="b8d8e-414">它可確保您不會在載入不一致的檔案集時產生風險，例如，當使用者在下載應用程式檔的過程中，將新的部署套用至您的 web 伺服器。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-414">It ensures you don't risk loading an inconsistent set of files, for example if a new deployment is applied to your web server while the user is in the process of downloading the application files.</span></span> <span data-ttu-id="b8d8e-415">不一致的檔案可能會導致未定義的行為。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-415">Inconsistent files could lead to undefined behavior.</span></span>
* <span data-ttu-id="b8d8e-416">它可確保使用者的瀏覽器永遠不會快取不一致或不正確回應，這可能會讓他們無法啟動應用程式，即使它們手動重新整理頁面也是如此。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-416">It ensures the user's browser never caches inconsistent or invalid responses, which could prevent them from starting the app even if they manually refresh the page.</span></span>
* <span data-ttu-id="b8d8e-417">這可讓您安全地快取回應，甚至不檢查伺服器端的變更，直到預期的 256 SHA-1 雜湊本身變更為止，因此後續頁面載入牽涉到較少的要求，並更快完成。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-417">It makes it safe to cache the responses and not even check for server-side changes until the expected SHA-256 hashes themselves change, so subsequent page loads involve fewer requests and complete much faster.</span></span>

<span data-ttu-id="b8d8e-418">如果您的 web 伺服器傳回的回應不符合預期的 256 SHA-1 雜湊，您將會在瀏覽器的開發人員主控台中看到類似下面的錯誤：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-418">If your web server returns responses that don't match the expected SHA-256 hashes, you will see an error similar to the following appear in the browser's developer console:</span></span>

> <span data-ttu-id="b8d8e-419">在 https://myapp.example.com/\_framework/My Blazor 計算 SHA-256 完整性 ' IIa70iwvmEg5WiDV17OpQ5eCztNYqL186J56852RpJY = ' 的資源 'App.dll ' 的 ' 完整性 ' 屬性中找不到有效的摘要。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-419">Failed to find a valid digest in the 'integrity' attribute for resource 'https://myapp.example.com/\_framework/MyBlazorApp.dll' with computed SHA-256 integrity 'IIa70iwvmEg5WiDV17OpQ5eCztNYqL186J56852RpJY='.</span></span> <span data-ttu-id="b8d8e-420">資源已被封鎖。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-420">The resource has been blocked.</span></span>

<span data-ttu-id="b8d8e-421">在大部分的情況下，這 *不* 是完整性檢查本身的問題。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-421">In most cases, this is *not* a problem with integrity checking itself.</span></span> <span data-ttu-id="b8d8e-422">相反地，這表示有其他問題，而完整性檢查則會警告您有關該其他問題。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-422">Instead, it means there is some other problem, and the integrity check is warning you about that other problem.</span></span>

### <a name="diagnosing-integrity-problems"></a><span data-ttu-id="b8d8e-423">診斷完整性問題</span><span class="sxs-lookup"><span data-stu-id="b8d8e-423">Diagnosing integrity problems</span></span>

<span data-ttu-id="b8d8e-424">建立應用程式時，產生的 `blazor.boot.json` 資訊清單會描述您開機資源的 256 sha-1 雜湊 (例如，、 `.dll` 以及 `.wasm` 產生組建輸出時) 的其他檔案。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-424">When an app is built, the generated `blazor.boot.json` manifest describes the SHA-256 hashes of your boot resources (for example, `.dll`, `.wasm`, and other files) at the time that the build output is produced.</span></span> <span data-ttu-id="b8d8e-425">只要 SHA-256 雜湊 `blazor.boot.json` 符合傳遞給瀏覽器的檔案，完整性檢查就會通過。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-425">The integrity check passes as long as the SHA-256 hashes in `blazor.boot.json` match the files delivered to the browser.</span></span>

<span data-ttu-id="b8d8e-426">這種失敗的常見原因如下：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-426">Common reasons why this fails are:</span></span>

 * <span data-ttu-id="b8d8e-427">Web 服務器的回應是錯誤 (例如， *找不到 404* 或 *500-Internal server 錯誤*) ，而不是瀏覽器要求的檔案。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-427">The web server's response is an error (for example, a *404 - Not Found* or a *500 - Internal Server Error*) instead of the file the browser requested.</span></span> <span data-ttu-id="b8d8e-428">瀏覽器會將此報告為完整性檢查失敗，而不是回應失敗。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-428">This is reported by the browser as an integrity check failure and not as a response failure.</span></span>
 * <span data-ttu-id="b8d8e-429">在檔案的組建和傳遞之間，檔案的內容已變更至瀏覽器。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-429">Something has changed the contents of the files between the build and delivery of the files to the browser.</span></span> <span data-ttu-id="b8d8e-430">這可能會發生：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-430">This might happen:</span></span>
   * <span data-ttu-id="b8d8e-431">如果您或組建工具手動修改組建輸出。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-431">If you or build tools manually modify the build output.</span></span>
   * <span data-ttu-id="b8d8e-432">如果部署程式的某些層面修改了檔案。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-432">If some aspect of the deployment process modified the files.</span></span> <span data-ttu-id="b8d8e-433">例如，如果您使用以 Git 為基礎的部署機制，請記住，如果您在 Windows 上認可檔案並在 Linux 上查看檔案，則 Git 會以透明的方式將 Windows 樣式的行尾結束符號轉換為 Unix 樣式的行尾結束符號。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-433">For example if you use a Git-based deployment mechanism, bear in mind that Git transparently converts Windows-style line endings to Unix-style line endings if you commit files on Windows and check them out on Linux.</span></span> <span data-ttu-id="b8d8e-434">變更檔案行尾結束符號會變更 256 SHA-1 雜湊。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-434">Changing file line endings change the SHA-256 hashes.</span></span> <span data-ttu-id="b8d8e-435">若要避免這個問題，請考慮 [使用 `.gitattributes` 將組建構件 `binary` 視為](https://git-scm.com/book/en/v2/Customizing-Git-Git-Attributes)檔案。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-435">To avoid this problem, consider [using `.gitattributes` to treat build artifacts as `binary` files](https://git-scm.com/book/en/v2/Customizing-Git-Git-Attributes).</span></span>
   * <span data-ttu-id="b8d8e-436">Web 服務器會在提供檔案內容的過程中修改檔案內容。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-436">The web server modifies the file contents as part of serving them.</span></span> <span data-ttu-id="b8d8e-437">例如，某些內容散發網路 (Cdn) 會自動嘗試 [縮短](xref:client-side/bundling-and-minification#minification) HTML，進而修改它。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-437">For example, some content distribution networks (CDNs) automatically attempt to [minify](xref:client-side/bundling-and-minification#minification) HTML, thereby modifying it.</span></span> <span data-ttu-id="b8d8e-438">您可能需要停用這類功能。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-438">You may need to disable such features.</span></span>

<span data-ttu-id="b8d8e-439">若要診斷下列哪一個適用于您的案例：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-439">To diagnose which of these applies in your case:</span></span>

 1. <span data-ttu-id="b8d8e-440">注意哪個檔案會藉由讀取錯誤訊息來觸發錯誤。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-440">Note which file is triggering the error by reading the error message.</span></span>
 1. <span data-ttu-id="b8d8e-441">開啟瀏覽器的開發人員工具，並查看 [ *網路* ] 索引標籤。如有必要，請重載頁面以查看要求和回應的清單。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-441">Open your browser's developer tools and look in the *Network* tab. If necessary, reload the page to see the list of requests and responses.</span></span> <span data-ttu-id="b8d8e-442">尋找在該清單中觸發錯誤的檔案。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-442">Find the file that is triggering the error in that list.</span></span>
 1. <span data-ttu-id="b8d8e-443">檢查回應中的 HTTP 狀態碼。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-443">Check the HTTP status code in the response.</span></span> <span data-ttu-id="b8d8e-444">如果伺服器傳回 *200-OK* (或另一個2xx 狀態碼) 以外的任何一個，則表示您有伺服器端的問題要進行診斷。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-444">If the server returns anything other than *200 - OK* (or another 2xx status code), then you have a server-side problem to diagnose.</span></span> <span data-ttu-id="b8d8e-445">例如，狀態碼403表示有授權問題，而狀態碼500表示伺服器以未指定的方式失敗。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-445">For example, status code 403 means there's an authorization problem, whereas status code 500 means the server is failing in an unspecified manner.</span></span> <span data-ttu-id="b8d8e-446">請參閱伺服器端記錄來診斷並修正應用程式。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-446">Consult server-side logs to diagnose and fix the app.</span></span>
 1. <span data-ttu-id="b8d8e-447">如果資源的狀態碼是 *200-確定* ，請查看瀏覽器開發人員工具中的回應內容，並檢查內容是否符合預期的資料。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-447">If the status code is *200 - OK* for the resource, look at the response content in browser's developer tools and check that the content matches up with the data expected.</span></span> <span data-ttu-id="b8d8e-448">例如，常見的問題是 jeffv 路由，讓要求即使是其他檔案也會傳回您的 `index.html` 資料。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-448">For example, a common problem is to misconfigure routing so that requests return your `index.html` data even for other files.</span></span> <span data-ttu-id="b8d8e-449">請確定要求的回應 `.wasm` 是 WebAssembly 二進位檔，而要求的回應 `.dll` 是 .net 元件二進位檔。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-449">Make sure that responses to `.wasm` requests are WebAssembly binaries and that responses to `.dll` requests are .NET assembly binaries.</span></span> <span data-ttu-id="b8d8e-450">如果沒有，您就有伺服器端路由問題需要進行診斷。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-450">If not, you have a server-side routing problem to diagnose.</span></span>
 1. <span data-ttu-id="b8d8e-451">搜尋以使用 [疑難排解完整性 PowerShell 腳本](#troubleshoot-integrity-powershell-script)來驗證應用程式的已發佈和已部署輸出。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-451">Seek to validate the app's published and deployed output with the [Troubleshoot integrity PowerShell script](#troubleshoot-integrity-powershell-script).</span></span>

<span data-ttu-id="b8d8e-452">如果您確認伺服器傳回義正辭嚴正確的資料，就必須在檔案的組建和傳遞之間修改內容。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-452">If you confirm that the server is returning plausibly correct data, there must be something else modifying the contents in between build and delivery of the file.</span></span> <span data-ttu-id="b8d8e-453">若要調查這一點：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-453">To investigate this:</span></span>

 * <span data-ttu-id="b8d8e-454">檢查組建工具鏈和部署機制，以防在建立檔案之後修改檔案。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-454">Examine the build toolchain and deployment mechanism in case they're modifying files after the files are built.</span></span> <span data-ttu-id="b8d8e-455">例如，如先前所述，Git 會轉換檔案行尾結束符號。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-455">An example of this is when Git transforms file line endings, as described earlier.</span></span>
 * <span data-ttu-id="b8d8e-456">檢查 web 伺服器或 CDN 設定，以防它們設定為動態修改回應 (例如，嘗試縮短 HTML) 。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-456">Examine the web server or CDN configuration in case they're set up to modify responses dynamically (for example, trying to minify HTML).</span></span> <span data-ttu-id="b8d8e-457">Web 服務器可正常執行 HTTP 壓縮 (例如，傳回 `content-encoding: br` 或 `content-encoding: gzip`) ，因為這不會影響解壓縮之後的結果。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-457">It's fine for the web server to implement HTTP compression (for example, returning `content-encoding: br` or `content-encoding: gzip`), since this doesn't affect the result after decompression.</span></span> <span data-ttu-id="b8d8e-458">不過，網頁伺服器 *不* 能修改未壓縮的資料。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-458">However, it's *not* fine for the web server to modify the uncompressed data.</span></span>

### <a name="troubleshoot-integrity-powershell-script"></a><span data-ttu-id="b8d8e-459">針對完整性 PowerShell 腳本進行疑難排解</span><span class="sxs-lookup"><span data-stu-id="b8d8e-459">Troubleshoot integrity PowerShell script</span></span>

<span data-ttu-id="b8d8e-460">使用 [`integrity.ps1`](https://github.com/dotnet/AspNetCore.Docs/blob/master/aspnetcore/blazor/host-and-deploy/webassembly/_samples/integrity.ps1?raw=true) PowerShell 腳本來驗證已發佈和已部署的 Blazor 應用程式。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-460">Use the [`integrity.ps1`](https://github.com/dotnet/AspNetCore.Docs/blob/master/aspnetcore/blazor/host-and-deploy/webassembly/_samples/integrity.ps1?raw=true) PowerShell script to validate a published and deployed Blazor app.</span></span> <span data-ttu-id="b8d8e-461">當應用程式有架構無法識別的完整性問題時，就會提供此腳本作為起點 Blazor 。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-461">The script is provided as a starting point when the app has integrity issues that the Blazor framework can't identify.</span></span> <span data-ttu-id="b8d8e-462">您的應用程式可能需要自訂腳本。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-462">Customization of the script might be required for your apps.</span></span>

<span data-ttu-id="b8d8e-463">腳本會檢查資料夾中的檔案 `publish` ，並從已部署的應用程式下載，以偵測包含完整性雜湊之不同資訊清單中的問題。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-463">The script checks the files in the `publish` folder and downloaded from the deployed app to detect issues in the different manifests that contain integrity hashes.</span></span> <span data-ttu-id="b8d8e-464">這些檢查應該會偵測到最常見的問題：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-464">These checks should detect the most common problems:</span></span>

* <span data-ttu-id="b8d8e-465">您已在已發行的輸出中修改檔案，但未加以察覺。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-465">You modified a file in the published output without realizing it.</span></span>
* <span data-ttu-id="b8d8e-466">應用程式未正確部署至部署目標，或部署目標環境內的某個變更。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-466">The app wasn't correctly deployed to the deployment target, or something changed within the deployment target's environment.</span></span>
* <span data-ttu-id="b8d8e-467">已部署的應用程式和發行應用程式的輸出之間有一些差異。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-467">There are differences between the deployed app and the output from publishing the app.</span></span>

<span data-ttu-id="b8d8e-468">在 PowerShell 命令 shell 中使用下列命令叫用腳本：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-468">Invoke the script with the following command in a PowerShell command shell:</span></span>

```powershell
.\integrity.ps1 {BASE URL} {PUBLISH OUTPUT FOLDER}
```

<span data-ttu-id="b8d8e-469">占 位 符：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-469">Placeholders:</span></span>

* <span data-ttu-id="b8d8e-470">`{BASE URL}`：已部署應用程式的 URL。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-470">`{BASE URL}`: The URL of the deployed app.</span></span>
* <span data-ttu-id="b8d8e-471">`{PUBLISH OUTPUT FOLDER}`：應用程式用來 `publish` 部署應用程式的資料夾或位置的路徑。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-471">`{PUBLISH OUTPUT FOLDER}`: The path to the app's `publish` folder or location where the app is published for deployment.</span></span>

> [!NOTE]
> <span data-ttu-id="b8d8e-472">若要將 `dotnet/AspNetCore.Docs` GitHub 存放庫複製到使用 [Bitdefender](https://www.bitdefender.com) 病毒掃描程式的系統，請將例外狀況新增至 `integrity.ps1` 腳本的 Bitdefender。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-472">To clone the `dotnet/AspNetCore.Docs` GitHub repository to a system that uses the [Bitdefender](https://www.bitdefender.com) virus scanner, add an exception to Bitdefender for the `integrity.ps1` script.</span></span> <span data-ttu-id="b8d8e-473">在複製存放庫之前，將例外狀況新增至 Bitdefender，以避免病毒掃描程式隔離腳本。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-473">Add the exception to Bitdefender before cloning the repo to avoid having the script quarantined by the virus scanner.</span></span> <span data-ttu-id="b8d8e-474">下列範例是 Windows 系統上複製之存放庫的腳本一般路徑。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-474">The following example is a typical path to the script for the cloned repo on a Windows system.</span></span> <span data-ttu-id="b8d8e-475">視需要調整路徑。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-475">Adjust the path as needed.</span></span> <span data-ttu-id="b8d8e-476">預留位置 `{USER}` 是使用者的路徑區段。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-476">The placeholder `{USER}` is the user's path segment.</span></span>
>
> ```
> C:\Users\{USER}\Documents\GitHub\AspNetCore.Docs\aspnetcore\blazor\host-and-deploy\webassembly\_samples\integrity.ps1
> ```

### <a name="disable-integrity-checking-for-non-pwa-apps"></a><span data-ttu-id="b8d8e-477">停用非 PWA 應用程式的完整性檢查</span><span class="sxs-lookup"><span data-stu-id="b8d8e-477">Disable integrity checking for non-PWA apps</span></span>

<span data-ttu-id="b8d8e-478">在大多數情況下，請勿停用完整性檢查。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-478">In most cases, don't disable integrity checking.</span></span> <span data-ttu-id="b8d8e-479">停用完整性檢查並無法解決造成非預期回應的根本問題，因而導致遺失先前所列的權益。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-479">Disabling integrity checking doesn't solve the underlying problem that has caused the unexpected responses and results in losing the benefits listed earlier.</span></span>

<span data-ttu-id="b8d8e-480">在某些情況下，無法依賴網頁伺服器傳回一致的回應，而且您沒有任何選擇，而是停用完整性檢查。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-480">There may be cases where the web server can't be relied upon to return consistent responses, and you have no choice but to disable integrity checks.</span></span> <span data-ttu-id="b8d8e-481">若要停用完整性檢查，請將下列內容新增至專案檔中的屬性群組 Blazor WebAssembly `.csproj` ：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-481">To disable integrity checks, add the following to a property group in the Blazor WebAssembly project's `.csproj` file:</span></span>

```xml
<BlazorCacheBootResources>false</BlazorCacheBootResources>
```

<span data-ttu-id="b8d8e-482">`BlazorCacheBootResources` 也會 Blazor `.dll` 根據其 256 sha-1 雜湊停用快取、和其他檔案的預設行為， `.wasm` 因為屬性指出無法依賴 sha-256 雜湊來取得正確性。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-482">`BlazorCacheBootResources` also disables Blazor's default behavior of caching the `.dll`, `.wasm`, and other files based on their SHA-256 hashes because the property indicates that the SHA-256 hashes can't be relied upon for correctness.</span></span> <span data-ttu-id="b8d8e-483">即使使用此設定，瀏覽器的一般 HTTP 快取仍可能會快取這些檔案，但這是否發生，取決於您的 web 伺服器設定和它所提供的 `cache-control` 標頭。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-483">Even with this setting, the browser's normal HTTP cache may still cache those files, but whether or not this happens depends on your web server configuration and the `cache-control` headers that it serves.</span></span>

> [!NOTE]
> <span data-ttu-id="b8d8e-484">`BlazorCacheBootResources`屬性不會停用[漸進式 Web 應用程式的完整性檢查 (pwa) ](xref:blazor/progressive-web-app)。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-484">The `BlazorCacheBootResources` property doesn't disable integrity checks for [Progressive Web Applications (PWAs)](xref:blazor/progressive-web-app).</span></span> <span data-ttu-id="b8d8e-485">如需 Pwa 的相關指引，請參閱 [停用 pwa 的完整性檢查](#disable-integrity-checking-for-pwas) 一節。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-485">For guidance pertaining to PWAs, see the [Disable integrity checking for PWAs](#disable-integrity-checking-for-pwas) section.</span></span>

### <a name="disable-integrity-checking-for-pwas"></a><span data-ttu-id="b8d8e-486">停用 Pwa 的完整性檢查</span><span class="sxs-lookup"><span data-stu-id="b8d8e-486">Disable integrity checking for PWAs</span></span>

<span data-ttu-id="b8d8e-487">Blazor的漸進式 Web 應用程式 (PWA) 範本包含建議的檔案 `service-worker.published.js` ，該檔案負責提取和儲存應用程式檔，以供離線使用。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-487">Blazor's Progressive Web Application (PWA) template contains a suggested `service-worker.published.js` file that's responsible for fetching and storing application files for offline use.</span></span> <span data-ttu-id="b8d8e-488">這是與一般應用程式啟動機制不同的程式，且具有自己的個別完整性檢查邏輯。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-488">This is a separate process from the normal app startup mechanism and has its own separate integrity checking logic.</span></span>

<span data-ttu-id="b8d8e-489">在檔案中 `service-worker.published.js` ，下列程式程式碼存在：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-489">Inside the `service-worker.published.js` file, following line is present:</span></span>

```javascript
.map(asset => new Request(asset.url, { integrity: asset.hash }));
```

<span data-ttu-id="b8d8e-490">若要停用完整性檢查，請將這 `integrity` 一行變更為下列程式碼來移除參數：</span><span class="sxs-lookup"><span data-stu-id="b8d8e-490">To disable integrity checking, remove the `integrity` parameter by changing the line to the following:</span></span>

```javascript
.map(asset => new Request(asset.url));
```

<span data-ttu-id="b8d8e-491">同樣地，停用完整性檢查即表示您遺失完整性檢查所提供的安全性保證。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-491">Again, disabling integrity checking means that you lose the safety guarantees offered by integrity checking.</span></span> <span data-ttu-id="b8d8e-492">例如，如果使用者的瀏覽器在您部署新版本的確切時刻快取了應用程式，則可能會有一個風險，它可以從舊的部署快取一些檔案，而有些則會從新的部署中快取。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-492">For example, there is a risk that if the user's browser is caching the app at the exact moment that you deploy a new version, it could cache some files from the old deployment and some from the new deployment.</span></span> <span data-ttu-id="b8d8e-493">如果發生這種情況，應用程式就會變成處於中斷狀態，直到您部署進一步的更新。</span><span class="sxs-lookup"><span data-stu-id="b8d8e-493">If that happens, the app becomes stuck in a broken state until you deploy a further update.</span></span>
