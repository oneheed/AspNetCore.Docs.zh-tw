...
* <span data-ttu-id="80af2-122">裝載在不支援靜態壓縮的檔案內容協商的靜態裝載方案（例如 GitHub 頁面）時，請考慮將應用程式設定為提取和解碼 Brotli 壓縮檔案：</span><span class="sxs-lookup"><span data-stu-id="80af2-122">When hosting on static hosting solutions that don't support statically-compressed file content negotiation, such as GitHub Pages, consider configuring the app to fetch and decode Brotli compressed files:</span></span>
  ...
  * <span data-ttu-id="80af2-123">從 [google/Brotli GitHub 存放庫](https://github.com/google/brotli)取得 JavaScript Brotli 解碼器。</span><span class="sxs-lookup"><span data-stu-id="80af2-123">Obtain the JavaScript Brotli decoder from the [google/brotli GitHub repository](https://github.com/google/brotli).</span></span> <span data-ttu-id="80af2-124">從2020年9月起， `decode.js` 系統會在存放庫的[ `js` 資料夾](https://github.com/google/brotli/tree/master/js)中命名並重命名的解碼器檔案。</span><span class="sxs-lookup"><span data-stu-id="80af2-124">As of September 2020, the decoder file is named `decode.js` and found in the repository's [`js` folder](https://github.com/google/brotli/tree/master/js).</span></span>
...
* <span data-ttu-id="80af2-164">將第二個用戶端應用程式新增至解決方案：</span><span class="sxs-lookup"><span data-stu-id="80af2-164">Add a second client app to the solution:</span></span>
  ...
  * <span data-ttu-id="80af2-167">在應用程式的專案檔中：</span><span class="sxs-lookup"><span data-stu-id="80af2-167">In the app's project file:</span></span>
    ...
    * <span data-ttu-id="80af2-169">將專案參考新增至 `Shared` 專案：</span><span class="sxs-lookup"><span data-stu-id="80af2-169">Add a project reference to the `Shared` project:</span></span>
...
* <span data-ttu-id="80af2-173">在伺服器應用程式的 `Startup.Configure` 方法 (`Startup.cs`) 中，移除下列幾行，這些行會在呼叫之後出現 <xref:Microsoft.AspNetCore.Builder.HttpsPolicyBuilderExtensions.UseHttpsRedirection%2A> ：</span><span class="sxs-lookup"><span data-stu-id="80af2-173">In the server app's `Startup.Configure` method (`Startup.cs`), remove the following lines, which appear after the call to <xref:Microsoft.AspNetCore.Builder.HttpsPolicyBuilderExtensions.UseHttpsRedirection%2A>:</span></span>
  ...
  * <span data-ttu-id="80af2-176">針對已新增的用戶端應用程式，要求埠可能是5001（適用于原始用戶端應用程式）或5002。</span><span class="sxs-lookup"><span data-stu-id="80af2-176">The request port is either 5001 for the original client app or 5002 for the added client app.</span></span>
...
* <span data-ttu-id="80af2-228">建立 URL Rewrite Module 規則：</span><span class="sxs-lookup"><span data-stu-id="80af2-228">URL Rewrite Module rules are established:</span></span>
  * <span data-ttu-id="80af2-229">提供應用程式的靜態資產位於 () 的子目錄 `wwwroot/{PATH REQUESTED}` 。</span><span class="sxs-lookup"><span data-stu-id="80af2-229">Serve the sub-directory where the app's static assets reside (`wwwroot/{PATH REQUESTED}`).</span></span>
...
#### <a name="configure-the-website"></a><span data-ttu-id="80af2-246">設定網站</span><span class="sxs-lookup"><span data-stu-id="80af2-246">Configure the website</span></span>
...
<span data-ttu-id="80af2-288">如需詳細資訊，請參閱 [Azure 儲存體中的靜態網站裝載](/azure/storage/blobs/storage-blob-static-website)。</span><span class="sxs-lookup"><span data-stu-id="80af2-288">For more information, see [Static website hosting in Azure Storage](/azure/storage/blobs/storage-blob-static-website).</span></span>
...
### <a name="urls"></a><span data-ttu-id="80af2-338">URL</span><span class="sxs-lookup"><span data-stu-id="80af2-338">URLs</span></span>
...
* <span data-ttu-id="80af2-367">URI 字串。</span><span class="sxs-lookup"><span data-stu-id="80af2-367">URI string.</span></span> <span data-ttu-id="80af2-368">在下列範例 (`wwwroot/index.html`) 中，從 CDN 提供下列檔案 `https://my-awesome-cdn.com/` ：</span><span class="sxs-lookup"><span data-stu-id="80af2-368">In the following example (`wwwroot/index.html`), the following files are served from a CDN at `https://my-awesome-cdn.com/`:</span></span>

  * `dotnet.*.js`
  * `dotnet.wasm`
  * <span data-ttu-id="80af2-369">時區資料</span><span class="sxs-lookup"><span data-stu-id="80af2-369">Timezone data</span></span>
...
* <span data-ttu-id="80af2-370">`Promise<Response>`.</span><span class="sxs-lookup"><span data-stu-id="80af2-370">`Promise<Response>`.</span></span> <span data-ttu-id="80af2-371">`integrity`在標頭中傳遞參數，以保留預設的完整性檢查行為。</span><span class="sxs-lookup"><span data-stu-id="80af2-371">Pass the `integrity` parameter in a header to retain the default integrity-checking behavior.</span></span>

  <span data-ttu-id="80af2-372">下列範例 (`wwwroot/index.html`) 將自訂 HTTP 標頭新增至輸出要求，並將 `integrity` 參數傳遞至 `fetch` 呼叫：</span><span class="sxs-lookup"><span data-stu-id="80af2-372">The following example (`wwwroot/index.html`) adds a custom HTTP header to the outbound requests and passes the `integrity` parameter through to the `fetch` call:</span></span>
...
* <span data-ttu-id="80af2-373">`null`/`undefined`，這會導致預設的載入行為。</span><span class="sxs-lookup"><span data-stu-id="80af2-373">`null`/`undefined`, which results in the default loading behavior.</span></span>

<span data-ttu-id="80af2-374">外部來源必須傳回瀏覽器所需的 CORS 標頭，以允許跨原始來源資源載入。</span><span class="sxs-lookup"><span data-stu-id="80af2-374">External sources must return the required CORS headers for browsers to allow the cross-origin resource loading.</span></span> <span data-ttu-id="80af2-375">依預設，Cdn 通常會提供必要的標頭。</span><span class="sxs-lookup"><span data-stu-id="80af2-375">CDNs usually provide the required headers by default.</span></span>

<span data-ttu-id="80af2-376">您只需要指定自訂行為的類型。</span><span class="sxs-lookup"><span data-stu-id="80af2-376">You only need to specify types for custom behaviors.</span></span> <span data-ttu-id="80af2-377">未指定的類型 `loadBootResource` 會根據其預設載入行為載入架構。</span><span class="sxs-lookup"><span data-stu-id="80af2-377">Types not specified to `loadBootResource` are loaded by the framework per their default loading behaviors.</span></span>

## <a name="change-the-filename-extension-of-dll-files"></a><span data-ttu-id="80af2-378">變更 DLL 檔案的副檔名</span><span class="sxs-lookup"><span data-stu-id="80af2-378">Change the filename extension of DLL files</span></span>

<span data-ttu-id="80af2-379">如果您需要變更應用程式已發佈檔案的副檔名 `.dll` ，請遵循本節中的指導方針。</span><span class="sxs-lookup"><span data-stu-id="80af2-379">In case you have a need to change the filename extensions of the app's published `.dll` files, follow the guidance in this section.</span></span>

<span data-ttu-id="80af2-380">發佈應用程式之後，請使用 shell 腳本或 DevOps 組建管線，將檔案重新命名 `.dll` 以使用不同的副檔名。</span><span class="sxs-lookup"><span data-stu-id="80af2-380">After publishing the app, use a shell script or DevOps build pipeline to rename `.dll` files to use a different file extension.</span></span> <span data-ttu-id="80af2-381">以 `.dll` `wwwroot` 應用程式發佈輸出的目錄中的檔案為目標 (例如 `{CONTENT ROOT}/bin/Release/netstandard2.1/publish/wwwroot`) 。</span><span class="sxs-lookup"><span data-stu-id="80af2-381">Target the `.dll` files in the `wwwroot` directory of the app's published output (for example, `{CONTENT ROOT}/bin/Release/netstandard2.1/publish/wwwroot`).</span></span>

<span data-ttu-id="80af2-382">在下列範例中， `.dll` 會將檔案重新命名為使用 `.bin` 副檔名。</span><span class="sxs-lookup"><span data-stu-id="80af2-382">In the following examples, `.dll` files are renamed to use the `.bin` file extension.</span></span>

<span data-ttu-id="80af2-383">在 Windows 上：</span><span class="sxs-lookup"><span data-stu-id="80af2-383">On Windows:</span></span>
...
<span data-ttu-id="80af2-384">如果服務工作者資產也在使用中，請新增下列命令：</span><span class="sxs-lookup"><span data-stu-id="80af2-384">If service worker assets are also in use, add the following command:</span></span>

```powershell
((Get-Content .\service-worker-assets.js -Raw) -replace '.dll"','.bin"') | Set-Content .\service-worker-assets.js
```

<span data-ttu-id="80af2-385">在 Linux 或 macOS 上：</span><span class="sxs-lookup"><span data-stu-id="80af2-385">On Linux or macOS:</span></span>
...
<span data-ttu-id="80af2-386">如果服務工作者資產也在使用中，請新增下列命令：</span><span class="sxs-lookup"><span data-stu-id="80af2-386">If service worker assets are also in use, add the following command:</span></span>

```console
sed -i 's/\.dll"/.bin"/g' service-worker-assets.js
```
   
<span data-ttu-id="80af2-387">若要使用不同的副檔名 `.bin` ，請 `.bin` 在上述命令中取代。</span><span class="sxs-lookup"><span data-stu-id="80af2-387">To use a different file extension than `.bin`, replace `.bin` in the preceding commands.</span></span>

<span data-ttu-id="80af2-388">若要處理壓縮 `blazor.boot.json.gz` 和檔案 `blazor.boot.json.br` ，請採用下列其中一種方法：</span><span class="sxs-lookup"><span data-stu-id="80af2-388">To address the compressed `blazor.boot.json.gz` and `blazor.boot.json.br` files, adopt either of the following approaches:</span></span>

* <span data-ttu-id="80af2-389">移除壓縮的 `blazor.boot.json.gz` 和檔案 `blazor.boot.json.br` 。</span><span class="sxs-lookup"><span data-stu-id="80af2-389">Remove the compressed `blazor.boot.json.gz` and `blazor.boot.json.br` files.</span></span> <span data-ttu-id="80af2-390">這種方法會停用壓縮。</span><span class="sxs-lookup"><span data-stu-id="80af2-390">Compression is disabled with this approach.</span></span>
* <span data-ttu-id="80af2-391">重新壓縮更新後的檔案 `blazor.boot.json` 。</span><span class="sxs-lookup"><span data-stu-id="80af2-391">Recompress the updated `blazor.boot.json` file.</span></span>

<span data-ttu-id="80af2-392">先前的指導方針也適用于服務工作者資產正在使用中。</span><span class="sxs-lookup"><span data-stu-id="80af2-392">The preceding guidance also applies when service worker assets are in use.</span></span> <span data-ttu-id="80af2-393">移除或重新壓縮 `wwwroot/service-worker-assets.js.br` 和 `wwwroot/service-worker-assets.js.gz` 。</span><span class="sxs-lookup"><span data-stu-id="80af2-393">Remove or recompress `wwwroot/service-worker-assets.js.br` and `wwwroot/service-worker-assets.js.gz`.</span></span> <span data-ttu-id="80af2-394">否則，瀏覽器中的檔案完整性檢查會失敗。</span><span class="sxs-lookup"><span data-stu-id="80af2-394">Otherwise, file integrity checks fail in the browser.</span></span>

<span data-ttu-id="80af2-395">下列 Windows 範例使用放置於專案根目錄的 PowerShell 腳本。</span><span class="sxs-lookup"><span data-stu-id="80af2-395">The following Windows example uses a PowerShell script placed at the root of the project.</span></span>

<span data-ttu-id="80af2-396">`ChangeDLLExtensions.ps1:`:</span><span class="sxs-lookup"><span data-stu-id="80af2-396">`ChangeDLLExtensions.ps1:`:</span></span>
...
<span data-ttu-id="80af2-397">如果服務工作者資產也在使用中，請新增下列命令：</span><span class="sxs-lookup"><span data-stu-id="80af2-397">If service worker assets are also in use, add the following command:</span></span>

```powershell
((Get-Content $filepath\bin\Release\$tfm\wwwroot\service-worker-assets.js -Raw) -replace '.dll"','.bin"') | Set-Content $filepath\bin\Release\$tfm\wwwroot\service-worker-assets.js
```

<span data-ttu-id="80af2-398">在專案檔中，腳本會在發佈應用程式之後執行：</span><span class="sxs-lookup"><span data-stu-id="80af2-398">In the project file, the script is run after publishing the app:</span></span>
...
> [!NOTE]
> <span data-ttu-id="80af2-399">重新命名和消極式載入相同的元件時，請參閱中的指導方針 <xref:blazor/webassembly-lazy-load-assemblies#onnavigateasync-events-and-renamed-assembly-files> 。</span><span class="sxs-lookup"><span data-stu-id="80af2-399">When renaming and lazy loading the same assemblies, see the guidance in <xref:blazor/webassembly-lazy-load-assemblies#onnavigateasync-events-and-renamed-assembly-files>.</span></span>
...
* <span data-ttu-id="80af2-407">這可讓您安全地快取回應，甚至不檢查伺服器端的變更，直到預期的 256 SHA-1 雜湊本身變更為止，因此後續頁面載入牽涉到較少的要求，並更快完成。</span><span class="sxs-lookup"><span data-stu-id="80af2-407">It makes it safe to cache the responses and not even check for server-side changes until the expected SHA-256 hashes themselves change, so subsequent page loads involve fewer requests and complete much faster.</span></span>
...
### <a name="diagnosing-integrity-problems"></a><span data-ttu-id="80af2-411">診斷完整性問題</span><span class="sxs-lookup"><span data-stu-id="80af2-411">Diagnosing integrity problems</span></span>

<span data-ttu-id="80af2-412">建立應用程式時，產生的 `blazor.boot.json` 資訊清單會描述您開機資源的 256 sha-1 雜湊 (例如，、 `.dll` 以及 `.wasm` 產生組建輸出時) 的其他檔案。</span><span class="sxs-lookup"><span data-stu-id="80af2-412">When an app is built, the generated `blazor.boot.json` manifest describes the SHA-256 hashes of your boot resources (for example, `.dll`, `.wasm`, and other files) at the time that the build output is produced.</span></span> <span data-ttu-id="80af2-413">只要 SHA-256 雜湊 `blazor.boot.json` 符合傳遞給瀏覽器的檔案，完整性檢查就會通過。</span><span class="sxs-lookup"><span data-stu-id="80af2-413">The integrity check passes as long as the SHA-256 hashes in `blazor.boot.json` match the files delivered to the browser.</span></span>

<span data-ttu-id="80af2-414">這種失敗的常見原因如下：</span><span class="sxs-lookup"><span data-stu-id="80af2-414">Common reasons why this fails are:</span></span>

 * <span data-ttu-id="80af2-415">Web 服務器的回應是錯誤 (例如， *找不到 404* 或 *500-Internal server 錯誤* ) ，而不是瀏覽器要求的檔案。</span><span class="sxs-lookup"><span data-stu-id="80af2-415">The web server's response is an error (for example, a *404 - Not Found* or a *500 - Internal Server Error* ) instead of the file the browser requested.</span></span> <span data-ttu-id="80af2-416">瀏覽器會將此報告為完整性檢查失敗，而不是回應失敗。</span><span class="sxs-lookup"><span data-stu-id="80af2-416">This is reported by the browser as an integrity check failure and not as a response failure.</span></span>
 * <span data-ttu-id="80af2-417">在檔案的組建和傳遞之間，檔案的內容已變更至瀏覽器。</span><span class="sxs-lookup"><span data-stu-id="80af2-417">Something has changed the contents of the files between the build and delivery of the files to the browser.</span></span> <span data-ttu-id="80af2-418">這可能會發生：</span><span class="sxs-lookup"><span data-stu-id="80af2-418">This might happen:</span></span>
   * <span data-ttu-id="80af2-419">如果您或組建工具手動修改組建輸出。</span><span class="sxs-lookup"><span data-stu-id="80af2-419">If you or build tools manually modify the build output.</span></span>
   * <span data-ttu-id="80af2-420">如果部署程式的某些層面修改了檔案。</span><span class="sxs-lookup"><span data-stu-id="80af2-420">If some aspect of the deployment process modified the files.</span></span> <span data-ttu-id="80af2-421">例如，如果您使用以 Git 為基礎的部署機制，請記住，如果您在 Windows 上認可檔案並在 Linux 上查看檔案，則 Git 會以透明的方式將 Windows 樣式的行尾結束符號轉換為 Unix 樣式的行尾結束符號。</span><span class="sxs-lookup"><span data-stu-id="80af2-421">For example if you use a Git-based deployment mechanism, bear in mind that Git transparently converts Windows-style line endings to Unix-style line endings if you commit files on Windows and check them out on Linux.</span></span> <span data-ttu-id="80af2-422">變更檔案行尾結束符號會變更 256 SHA-1 雜湊。</span><span class="sxs-lookup"><span data-stu-id="80af2-422">Changing file line endings change the SHA-256 hashes.</span></span> <span data-ttu-id="80af2-423">若要避免這個問題，請考慮 [使用 `.gitattributes` 將組建構件 `binary` 視為](https://git-scm.com/book/en/v2/Customizing-Git-Git-Attributes)檔案。</span><span class="sxs-lookup"><span data-stu-id="80af2-423">To avoid this problem, consider [using `.gitattributes` to treat build artifacts as `binary` files](https://git-scm.com/book/en/v2/Customizing-Git-Git-Attributes).</span></span>
   * <span data-ttu-id="80af2-424">Web 服務器會在提供檔案內容的過程中修改檔案內容。</span><span class="sxs-lookup"><span data-stu-id="80af2-424">The web server modifies the file contents as part of serving them.</span></span> <span data-ttu-id="80af2-425">例如，某些內容散發網路 (Cdn) 會自動嘗試 [縮短](xref:client-side/bundling-and-minification#minification) HTML，進而修改它。</span><span class="sxs-lookup"><span data-stu-id="80af2-425">For example, some content distribution networks (CDNs) automatically attempt to [minify](xref:client-side/bundling-and-minification#minification) HTML, thereby modifying it.</span></span> <span data-ttu-id="80af2-426">您可能需要停用這類功能。</span><span class="sxs-lookup"><span data-stu-id="80af2-426">You may need to disable such features.</span></span>

<span data-ttu-id="80af2-427">若要診斷下列哪一個適用于您的案例：</span><span class="sxs-lookup"><span data-stu-id="80af2-427">To diagnose which of these applies in your case:</span></span>

 1. <span data-ttu-id="80af2-428">注意哪個檔案會藉由讀取錯誤訊息來觸發錯誤。</span><span class="sxs-lookup"><span data-stu-id="80af2-428">Note which file is triggering the error by reading the error message.</span></span>
 1. <span data-ttu-id="80af2-429">開啟瀏覽器的開發人員工具，並查看 [ *網路* ] 索引標籤。如有必要，請重載頁面以查看要求和回應的清單。</span><span class="sxs-lookup"><span data-stu-id="80af2-429">Open your browser's developer tools and look in the *Network* tab. If necessary, reload the page to see the list of requests and responses.</span></span> <span data-ttu-id="80af2-430">尋找在該清單中觸發錯誤的檔案。</span><span class="sxs-lookup"><span data-stu-id="80af2-430">Find the file that is triggering the error in that list.</span></span>
 1. <span data-ttu-id="80af2-431">檢查回應中的 HTTP 狀態碼。</span><span class="sxs-lookup"><span data-stu-id="80af2-431">Check the HTTP status code in the response.</span></span> <span data-ttu-id="80af2-432">如果伺服器傳回 *200-OK* (或另一個2xx 狀態碼) 以外的任何一個，則表示您有伺服器端的問題要進行診斷。</span><span class="sxs-lookup"><span data-stu-id="80af2-432">If the server returns anything other than *200 - OK* (or another 2xx status code), then you have a server-side problem to diagnose.</span></span> <span data-ttu-id="80af2-433">例如，狀態碼403表示有授權問題，而狀態碼500表示伺服器以未指定的方式失敗。</span><span class="sxs-lookup"><span data-stu-id="80af2-433">For example, status code 403 means there's an authorization problem, whereas status code 500 means the server is failing in an unspecified manner.</span></span> <span data-ttu-id="80af2-434">請參閱伺服器端記錄來診斷並修正應用程式。</span><span class="sxs-lookup"><span data-stu-id="80af2-434">Consult server-side logs to diagnose and fix the app.</span></span>
 1. <span data-ttu-id="80af2-435">如果資源的狀態碼是 *200-確定* ，請查看瀏覽器開發人員工具中的回應內容，並檢查內容是否 matchs 預期的資料。</span><span class="sxs-lookup"><span data-stu-id="80af2-435">If the status code is *200 - OK* for the resource, look at the response content in browser's developer tools and check that the content matchs up with the data expected.</span></span> <span data-ttu-id="80af2-436">例如，常見的問題是 jeffv 路由，讓要求即使是其他檔案也會傳回您的 `index.html` 資料。</span><span class="sxs-lookup"><span data-stu-id="80af2-436">For example, a common problem is to misconfigure routing so that requests return your `index.html` data even for other files.</span></span> <span data-ttu-id="80af2-437">請確定要求的回應 `.wasm` 是 WebAssembly 二進位檔，而要求的回應 `.dll` 是 .net 元件二進位檔。</span><span class="sxs-lookup"><span data-stu-id="80af2-437">Make sure that responses to `.wasm` requests are WebAssembly binaries and that responses to `.dll` requests are .NET assembly binaries.</span></span> <span data-ttu-id="80af2-438">如果沒有，您就有伺服器端路由問題需要進行診斷。</span><span class="sxs-lookup"><span data-stu-id="80af2-438">If not, you have a server-side routing problem to diagnose.</span></span>

<span data-ttu-id="80af2-439">如果您確認伺服器傳回義正辭嚴正確的資料，就必須在檔案的組建和傳遞之間修改內容。</span><span class="sxs-lookup"><span data-stu-id="80af2-439">If you confirm that the server is returning plausibly correct data, there must be something else modifying the contents in between build and delivery of the file.</span></span> <span data-ttu-id="80af2-440">若要調查這一點：</span><span class="sxs-lookup"><span data-stu-id="80af2-440">To investigate this:</span></span>

 * <span data-ttu-id="80af2-441">檢查組建工具鏈和部署機制，以防在建立檔案之後修改檔案。</span><span class="sxs-lookup"><span data-stu-id="80af2-441">Examine the build toolchain and deployment mechanism in case they're modifying files after the files are built.</span></span> <span data-ttu-id="80af2-442">例如，如先前所述，Git 會轉換檔案行尾結束符號。</span><span class="sxs-lookup"><span data-stu-id="80af2-442">An example of this is when Git transforms file line endings, as described earlier.</span></span>
 * <span data-ttu-id="80af2-443">檢查 web 伺服器或 CDN 設定，以防它們設定為動態修改回應 (例如，嘗試縮短 HTML) 。</span><span class="sxs-lookup"><span data-stu-id="80af2-443">Examine the web server or CDN configuration in case they're set up to modify responses dynamically (for example, trying to minify HTML).</span></span> <span data-ttu-id="80af2-444">Web 服務器可正常執行 HTTP 壓縮 (例如，傳回 `content-encoding: br` 或 `content-encoding: gzip`) ，因為這不會影響解壓縮之後的結果。</span><span class="sxs-lookup"><span data-stu-id="80af2-444">It's fine for the web server to implement HTTP compression (for example, returning `content-encoding: br` or `content-encoding: gzip`), since this doesn't affect the result after decompression.</span></span> <span data-ttu-id="80af2-445">不過，網頁伺服器 *不* 能修改未壓縮的資料。</span><span class="sxs-lookup"><span data-stu-id="80af2-445">However, it's *not* fine for the web server to modify the uncompressed data.</span></span>

### <a name="disable-integrity-checking-for-non-pwa-apps"></a><span data-ttu-id="80af2-446">停用非 PWA 應用程式的完整性檢查</span><span class="sxs-lookup"><span data-stu-id="80af2-446">Disable integrity checking for non-PWA apps</span></span>
...
<span data-ttu-id="80af2-459">若要停用完整性檢查，請將這 `integrity` 一行變更為下列程式碼來移除參數：</span><span class="sxs-lookup"><span data-stu-id="80af2-459">To disable integrity checking, remove the `integrity` parameter by changing the line to the following:</span></span>

```javascript
.map(asset => new Request(asset.url));
```

<span data-ttu-id="80af2-460">同樣地，停用完整性檢查即表示您遺失完整性檢查所提供的安全性保證。</span><span class="sxs-lookup"><span data-stu-id="80af2-460">Again, disabling integrity checking means that you lose the safety guarantees offered by integrity checking.</span></span> <span data-ttu-id="80af2-461">例如，如果使用者的瀏覽器在您部署新版本的確切時刻快取了應用程式，則可能會有一個風險，它可以從舊的部署快取一些檔案，而有些則會從新的部署中快取。</span><span class="sxs-lookup"><span data-stu-id="80af2-461">For example, there is a risk that if the user's browser is caching the app at the exact moment that you deploy a new version, it could cache some files from the old deployment and some from the new deployment.</span></span> <span data-ttu-id="80af2-462">如果發生這種情況，應用程式就會變成處於中斷狀態，直到您部署進一步的更新。</span><span class="sxs-lookup"><span data-stu-id="80af2-462">If that happens, the app becomes stuck in a broken state until you deploy a further update.</span></span>
