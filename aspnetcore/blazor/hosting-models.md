...
Blazor WebAssembly建立應用程式以進行部署，而不使用後端 ASP.NET Core 應用程式來提供其檔案時，應用程式稱為 *獨立* Blazor WebAssembly 應用程式。 建立應用程式以供使用後端應用程式進行部署以提供其檔案時，應用程式稱為 *託管* Blazor WebAssembly 應用程式。 託管 Blazor WebAssembly 應用程式通常會使用 WEB API 呼叫或 () ，透過網路與伺服器互動 [SignalR](xref:signalr/introduction) <xref:tutorials/signalr-blazor-webassembly> 。
