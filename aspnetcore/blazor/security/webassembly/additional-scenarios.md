...
### <a name="custom-authorizationmessagehandler-class"></a><span data-ttu-id="9fc18-124">自訂 `AuthorizationMessageHandler` 類別</span><span class="sxs-lookup"><span data-stu-id="9fc18-124">Custom `AuthorizationMessageHandler` class</span></span>
...
## <a name="typed-httpclient"></a><span data-ttu-id="9fc18-143">類型 `HttpClient`</span><span class="sxs-lookup"><span data-stu-id="9fc18-143">Typed `HttpClient`</span></span>
...
<span data-ttu-id="9fc18-172">`Program.Main` (`Program.cs`):</span><span class="sxs-lookup"><span data-stu-id="9fc18-172">`Program.Main` (`Program.cs`):</span></span>
...
## <a name="cross-origin-resource-sharing-cors"></a><span data-ttu-id="9fc18-180">跨原始來源資源分享 (CORS)</span><span class="sxs-lookup"><span data-stu-id="9fc18-180">Cross-origin resource sharing (CORS)</span></span>
...
## <a name="handle-token-request-errors"></a><span data-ttu-id="9fc18-193">處理權杖要求錯誤</span><span class="sxs-lookup"><span data-stu-id="9fc18-193">Handle token request errors</span></span>
...
* <span data-ttu-id="9fc18-208">要求成功，且應用程式具有有效的權杖。</span><span class="sxs-lookup"><span data-stu-id="9fc18-208">The request succeeds, and the app has a valid token.</span></span>
...
<span data-ttu-id="9fc18-255">如果您選擇這麼做，就可以將 UI 分成不同的頁面。</span><span class="sxs-lookup"><span data-stu-id="9fc18-255">You're allowed to break the UI into different pages if you choose to do so.</span></span>
...
## <a name="customize-the-user"></a><span data-ttu-id="9fc18-264">自訂使用者</span><span class="sxs-lookup"><span data-stu-id="9fc18-264">Customize the user</span></span>
...
* <span data-ttu-id="9fc18-326">伺服器可以儲存重新整理權杖，並確保應用程式不會失去協力廠商資源的存取權。</span><span class="sxs-lookup"><span data-stu-id="9fc18-326">The server can store refresh tokens and ensure that the app doesn't lose access to third-party resources.</span></span>
...
<span data-ttu-id="9fc18-335">V2.0 端點的識別碼權杖中宣告的清單會變更。</span><span class="sxs-lookup"><span data-stu-id="9fc18-335">The list of claims in the ID token changes for v2.0 endpoints.</span></span> <span data-ttu-id="9fc18-336">如需詳細資訊，請參閱 [為何要更新至 Microsoft 身分識別平臺 (v2.0) ？](/azure/active-directory/azuread-dev/azure-ad-endpoint-comparison)。</span><span class="sxs-lookup"><span data-stu-id="9fc18-336">For more information, see [Why update to Microsoft identity platform (v2.0)?](/azure/active-directory/azuread-dev/azure-ad-endpoint-comparison).</span></span>

## <a name="configure-and-use-grpc-in-components"></a><span data-ttu-id="9fc18-337">在元件中設定和使用 gRPC</span><span class="sxs-lookup"><span data-stu-id="9fc18-337">Configure and use gRPC in components</span></span>

<span data-ttu-id="9fc18-338">若要將 Blazor WebAssembly 應用程式設定為使用 [ASP.NET Core gRPC 架構](xref:grpc/index)：</span><span class="sxs-lookup"><span data-stu-id="9fc18-338">To configure a Blazor WebAssembly app to use the [ASP.NET Core gRPC framework](xref:grpc/index):</span></span>

* <span data-ttu-id="9fc18-339">在伺服器上啟用 gRPC Web。</span><span class="sxs-lookup"><span data-stu-id="9fc18-339">Enable gRPC-Web on the server.</span></span> <span data-ttu-id="9fc18-340">如需詳細資訊，請參閱<xref:grpc/browser>。</span><span class="sxs-lookup"><span data-stu-id="9fc18-340">For more information, see <xref:grpc/browser>.</span></span>
* <span data-ttu-id="9fc18-341">註冊應用程式訊息處理常式的 gRPC 服務。</span><span class="sxs-lookup"><span data-stu-id="9fc18-341">Register gRPC services for the app's message handler.</span></span> <span data-ttu-id="9fc18-342">下列範例會設定應用程式的授權訊息處理常式，以使用[ `GreeterClient` gRPC 教學](xref:tutorials/grpc/grpc-start#create-a-grpc-service)課程 () 中的服務 `Program.Main` ：</span><span class="sxs-lookup"><span data-stu-id="9fc18-342">The following example configures the app's authorization message handler to use the [`GreeterClient` service from the gRPC tutorial](xref:tutorials/grpc/grpc-start#create-a-grpc-service) (`Program.Main`):</span></span>
...
<span data-ttu-id="9fc18-343">預留位置 `{APP ASSEMBLY}` 是應用程式的元件名稱 (例如 `BlazorSample`) 。</span><span class="sxs-lookup"><span data-stu-id="9fc18-343">The placeholder `{APP ASSEMBLY}` is the app's assembly name (for example, `BlazorSample`).</span></span> <span data-ttu-id="9fc18-344">將檔案放 `.proto` 在 `Shared` 主控方案的專案中 Blazor 。</span><span class="sxs-lookup"><span data-stu-id="9fc18-344">Place the `.proto` file in the `Shared` project of the hosted Blazor solution.</span></span>

<span data-ttu-id="9fc18-345">用戶端應用程式中的元件可以使用 gRPC 用戶端 () 來進行 gRPC 呼叫 `Pages/Grpc.razor` ：</span><span class="sxs-lookup"><span data-stu-id="9fc18-345">A component in the client app can make gRPC calls using the gRPC client (`Pages/Grpc.razor`):</span></span>
...
<span data-ttu-id="9fc18-346">預留位置 `{APP ASSEMBLY}` 是應用程式的元件名稱 (例如 `BlazorSample`) 。</span><span class="sxs-lookup"><span data-stu-id="9fc18-346">The placeholder `{APP ASSEMBLY}` is the app's assembly name (for example, `BlazorSample`).</span></span> <span data-ttu-id="9fc18-347">若要使用 `Status.DebugException` 屬性，請使用 [Grpc .Net. Client](https://www.nuget.org/packages/Grpc.Net.Client) version 2.30.0 或更新版本。</span><span class="sxs-lookup"><span data-stu-id="9fc18-347">To use the `Status.DebugException` property, use [Grpc.Net.Client](https://www.nuget.org/packages/Grpc.Net.Client) version 2.30.0 or later.</span></span>

<span data-ttu-id="9fc18-348">如需詳細資訊，請參閱<xref:grpc/browser>。</span><span class="sxs-lookup"><span data-stu-id="9fc18-348">For more information, see <xref:grpc/browser>.</span></span>

## <a name="build-a-custom-version-of-the-authenticationmsal-javascript-library"></a><span data-ttu-id="9fc18-349">建立驗證的自訂版本 MSAL JavaScript 程式庫</span><span class="sxs-lookup"><span data-stu-id="9fc18-349">Build a custom version of the Authentication.MSAL JavaScript library</span></span>

<span data-ttu-id="9fc18-350">如果應用程式需要 [適用于 JavaScript 的 Microsoft 驗證程式庫 ](https://www.npmjs.com/package/@azure/msal-browser)自訂版本 (MSAL.js) ，請執行下列步驟：</span><span class="sxs-lookup"><span data-stu-id="9fc18-350">If an app requires a custom version of the [Microsoft Authentication Library for JavaScript (MSAL.js)](https://www.npmjs.com/package/@azure/msal-browser), perform the following steps:</span></span>

1. <span data-ttu-id="9fc18-351">確認系統具有最新的開發人員 .NET SDK，或從 .Net Core SDK 取得並安裝最新的開發人員 SDK [：安裝程式和二進位](https://github.com/dotnet/installer#installers-and-binaries)檔。</span><span class="sxs-lookup"><span data-stu-id="9fc18-351">Confirm the the system has the latest developer .NET SDK or obtain and install the latest developer SDK from [.NET Core SDK: Installers and Binaries](https://github.com/dotnet/installer#installers-and-binaries).</span></span> <span data-ttu-id="9fc18-352">此案例不需要設定內部 NuGet 摘要。</span><span class="sxs-lookup"><span data-stu-id="9fc18-352">Configuration of internal NuGet feeds isn't required for this scenario.</span></span>
1. <span data-ttu-id="9fc18-353">設定 GitHub 存放 `dotnet/aspnetcore` 庫，以在 [Build ASP.NET Core from Source](https://github.com/dotnet/aspnetcore/blob/main/docs/BuildFromSource.md)的檔進行開發。</span><span class="sxs-lookup"><span data-stu-id="9fc18-353">Set up the `dotnet/aspnetcore` GitHub repository for development per the docs at [Build ASP.NET Core from Source](https://github.com/dotnet/aspnetcore/blob/main/docs/BuildFromSource.md).</span></span> <span data-ttu-id="9fc18-354">分叉和複製或下載 [dotnet/Aspnetcore GitHub 存放庫](https://github.com/dotnet/aspnetcore)的 ZIP 封存。</span><span class="sxs-lookup"><span data-stu-id="9fc18-354">Fork and clone or download a ZIP archive of the [dotnet/aspnetcore GitHub repository](https://github.com/dotnet/aspnetcore).</span></span>
1. <span data-ttu-id="9fc18-355">開啟檔案 `src/Components/WebAssembly/Authentication.Msal/src/Interop/package.json` ，並設定所需的版本 `@azure/msal-browser` 。</span><span class="sxs-lookup"><span data-stu-id="9fc18-355">Open the the `src/Components/WebAssembly/Authentication.Msal/src/Interop/package.json` file and set the desired version of `@azure/msal-browser`.</span></span> <span data-ttu-id="9fc18-356">如需已發行的版本清單，請造訪 [ `@azure/msal-browser` npm 網站](https://www.npmjs.com/package/@azure/msal-browser)，然後選取 [**版本**] 索引標籤。</span><span class="sxs-lookup"><span data-stu-id="9fc18-356">For a list of released versions, visit the [`@azure/msal-browser` npm website](https://www.npmjs.com/package/@azure/msal-browser) and select the **Versions** tab.</span></span>
1. <span data-ttu-id="9fc18-357">`Authentication.Msal` `src/Components/WebAssembly/Authentication.Msal/src` 使用命令 shell 中的命令，在資料夾中建立專案 `yarn build` 。</span><span class="sxs-lookup"><span data-stu-id="9fc18-357">Build the `Authentication.Msal` project in the `src/Components/WebAssembly/Authentication.Msal/src` folder with the `yarn build` command in a command shell.</span></span>
1. <span data-ttu-id="9fc18-358">如果應用程式使用 [壓縮的資產 (Brotli/Gzip) ](xref:blazor/host-and-deploy/webassembly#compression)，請壓縮 `Interop/dist/Release/AuthenticationService.js` 檔案。</span><span class="sxs-lookup"><span data-stu-id="9fc18-358">If the app uses [compressed assets (Brotli/Gzip)](xref:blazor/host-and-deploy/webassembly#compression), compress the `Interop/dist/Release/AuthenticationService.js` file.</span></span>
1. <span data-ttu-id="9fc18-359">`AuthenticationService.js`從資料夾將檔案和壓縮版本 (`.br` / `.gz`) 複製到應用 `Interop/dist/Release` 程式的 `publish/wwwroot/_content/Microsoft.Authentication.WebAssembly.Msal` 已發佈資產中的應用程式資料夾。</span><span class="sxs-lookup"><span data-stu-id="9fc18-359">Copy the `AuthenticationService.js` file and compressed versions (`.br`/`.gz`) of the file, if produced, from the `Interop/dist/Release` folder into the app's `publish/wwwroot/_content/Microsoft.Authentication.WebAssembly.Msal` folder in the app's published assets.</span></span>

## <a name="additional-resources"></a><span data-ttu-id="9fc18-360">其他資源</span><span class="sxs-lookup"><span data-stu-id="9fc18-360">Additional resources</span></span>

* <xref:blazor/security/webassembly/graph-api>
* [<span data-ttu-id="9fc18-361">`HttpClient` 以及 `HttpRequestMessage` 使用 FETCH API 要求選項</span><span class="sxs-lookup"><span data-stu-id="9fc18-361">`HttpClient` and `HttpRequestMessage` with Fetch API request options</span></span>](xref:blazor/call-web-api#httpclient-and-httprequestmessage-with-fetch-api-request-options)
