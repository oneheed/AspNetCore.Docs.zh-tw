...
### <a name="configure-authorizationmessagehandler"></a><span data-ttu-id="6c5c3-135">設定 `AuthorizationMessageHandler`</span><span class="sxs-lookup"><span data-stu-id="6c5c3-135">Configure `AuthorizationMessageHandler`</span></span>
...
<span data-ttu-id="6c5c3-147">預留位置 `{APP ASSEMBLY}` 是應用程式的元件名稱 (例如 `using static BlazorSample.Data;`) 。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-147">The placeholder `{APP ASSEMBLY}` is the app's assembly name (for example, `using static BlazorSample.Data;`).</span></span>
...
<span data-ttu-id="6c5c3-178"><xref:Microsoft.AspNetCore.Components.WebAssembly.Authentication.AccessTokenResult.TryGetToken%2A?displayProperty=nameWithType> 返回：</span><span class="sxs-lookup"><span data-stu-id="6c5c3-178"><xref:Microsoft.AspNetCore.Components.WebAssembly.Authentication.AccessTokenResult.TryGetToken%2A?displayProperty=nameWithType> returns:</span></span>
...
<span data-ttu-id="6c5c3-208">當應用程式要求權杖時，有兩個可能的結果：</span><span class="sxs-lookup"><span data-stu-id="6c5c3-208">When the app requests a token, there are two possible outcomes:</span></span>
...
<span data-ttu-id="6c5c3-256">如果您選擇這麼做，就可以將 UI 分成不同的頁面。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-256">You're allowed to break the UI into different pages if you choose to do so.</span></span>
...
## <a name="customize-the-user"></a><span data-ttu-id="6c5c3-265">自訂使用者</span><span class="sxs-lookup"><span data-stu-id="6c5c3-265">Customize the user</span></span>
...
* <span data-ttu-id="6c5c3-327">伺服器可以儲存重新整理權杖，並確保應用程式不會失去協力廠商資源的存取權。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-327">The server can store refresh tokens and ensure that the app doesn't lose access to third-party resources.</span></span>
...
<span data-ttu-id="6c5c3-330">驗證程式庫和 Blazor 專案範本使用 OpenID Connect (OIDC) v1.0 端點。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-330">The authentication library and Blazor project templates use OpenID Connect (OIDC) v1.0 endpoints.</span></span> <span data-ttu-id="6c5c3-331">若要使用 v2.0 端點，請設定 JWT 持有人 <xref:Microsoft.AspNetCore.Builder.JwtBearerOptions.Authority?displayProperty=nameWithType> 選項。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-331">To use a v2.0 endpoint, configure the JWT Bearer <xref:Microsoft.AspNetCore.Builder.JwtBearerOptions.Authority?displayProperty=nameWithType> option.</span></span> <span data-ttu-id="6c5c3-332">在下列範例中，會將區段附加至屬性，以針對 v2.0 設定 AAD `v2.0` <xref:Microsoft.AspNetCore.Builder.JwtBearerOptions.Authority> ：</span><span class="sxs-lookup"><span data-stu-id="6c5c3-332">In the following example, AAD is configured for v2.0 by appending a `v2.0` segment to the <xref:Microsoft.AspNetCore.Builder.JwtBearerOptions.Authority> property:</span></span>
...
<span data-ttu-id="6c5c3-333">或者，您可以在應用程式設定 () 檔中進行設定 `appsettings.json` ：</span><span class="sxs-lookup"><span data-stu-id="6c5c3-333">Alternatively, the setting can be made in the app settings (`appsettings.json`) file:</span></span>
...
<span data-ttu-id="6c5c3-334">如果追蹤于區段上的授權不適合應用程式的 OIDC 提供者（例如，使用非 AAD 提供者），請 <xref:Microsoft.AspNetCore.Builder.OpenIdConnectOptions.Authority> 直接設定屬性。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-334">If tacking on a segment to the authority isn't appropriate for the app's OIDC provider, such as with non-AAD providers, set the <xref:Microsoft.AspNetCore.Builder.OpenIdConnectOptions.Authority> property directly.</span></span> <span data-ttu-id="6c5c3-335">您可以在 <xref:Microsoft.AspNetCore.Builder.JwtBearerOptions> 應用程式佈建檔案中設定或在應用程式佈建檔案中設定屬性 (`appsettings.json`) 的 `Authority` 金鑰。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-335">Either set the property in <xref:Microsoft.AspNetCore.Builder.JwtBearerOptions> or in the app settings file (`appsettings.json`) with the `Authority` key.</span></span>

<span data-ttu-id="6c5c3-336">V2.0 端點的識別碼權杖中宣告的清單會變更。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-336">The list of claims in the ID token changes for v2.0 endpoints.</span></span> <span data-ttu-id="6c5c3-337">如需詳細資訊，請參閱 [為何要更新至 Microsoft 身分識別平臺 (v2.0) ？](/azure/active-directory/azuread-dev/azure-ad-endpoint-comparison)。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-337">For more information, see [Why update to Microsoft identity platform (v2.0)?](/azure/active-directory/azuread-dev/azure-ad-endpoint-comparison).</span></span>

## <a name="configure-and-use-grpc-in-components"></a><span data-ttu-id="6c5c3-338">在元件中設定和使用 gRPC</span><span class="sxs-lookup"><span data-stu-id="6c5c3-338">Configure and use gRPC in components</span></span>

<span data-ttu-id="6c5c3-339">若要將 Blazor WebAssembly 應用程式設定為使用 [ASP.NET Core gRPC 架構](xref:grpc/index)：</span><span class="sxs-lookup"><span data-stu-id="6c5c3-339">To configure a Blazor WebAssembly app to use the [ASP.NET Core gRPC framework](xref:grpc/index):</span></span>

* <span data-ttu-id="6c5c3-340">在伺服器上啟用 gRPC Web。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-340">Enable gRPC-Web on the server.</span></span> <span data-ttu-id="6c5c3-341">如需詳細資訊，請參閱<xref:grpc/browser>。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-341">For more information, see <xref:grpc/browser>.</span></span>
* <span data-ttu-id="6c5c3-342">註冊應用程式訊息處理常式的 gRPC 服務。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-342">Register gRPC services for the app's message handler.</span></span> <span data-ttu-id="6c5c3-343">下列範例會設定應用程式的授權訊息處理常式，以使用[ `GreeterClient` gRPC 教學](xref:tutorials/grpc/grpc-start#create-a-grpc-service)課程 () 中的服務 `Program.Main` ：</span><span class="sxs-lookup"><span data-stu-id="6c5c3-343">The following example configures the app's authorization message handler to use the [`GreeterClient` service from the gRPC tutorial](xref:tutorials/grpc/grpc-start#create-a-grpc-service) (`Program.Main`):</span></span>
...
<span data-ttu-id="6c5c3-344">預留位置 `{APP ASSEMBLY}` 是應用程式的元件名稱 (例如 `BlazorSample`) 。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-344">The placeholder `{APP ASSEMBLY}` is the app's assembly name (for example, `BlazorSample`).</span></span> <span data-ttu-id="6c5c3-345">將檔案放 `.proto` 在 `Shared` 主控方案的專案中 Blazor 。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-345">Place the `.proto` file in the `Shared` project of the hosted Blazor solution.</span></span>

<span data-ttu-id="6c5c3-346">用戶端應用程式中的元件可以使用 gRPC 用戶端 () 來進行 gRPC 呼叫 `Pages/Grpc.razor` ：</span><span class="sxs-lookup"><span data-stu-id="6c5c3-346">A component in the client app can make gRPC calls using the gRPC client (`Pages/Grpc.razor`):</span></span>
...
<span data-ttu-id="6c5c3-347">預留位置 `{APP ASSEMBLY}` 是應用程式的元件名稱 (例如 `BlazorSample`) 。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-347">The placeholder `{APP ASSEMBLY}` is the app's assembly name (for example, `BlazorSample`).</span></span> <span data-ttu-id="6c5c3-348">若要使用 `Status.DebugException` 屬性，請使用 [Grpc .Net. Client](https://www.nuget.org/packages/Grpc.Net.Client) version 2.30.0 或更新版本。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-348">To use the `Status.DebugException` property, use [Grpc.Net.Client](https://www.nuget.org/packages/Grpc.Net.Client) version 2.30.0 or later.</span></span>

<span data-ttu-id="6c5c3-349">如需詳細資訊，請參閱<xref:grpc/browser>。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-349">For more information, see <xref:grpc/browser>.</span></span>

## <a name="build-a-custom-version-of-the-authenticationmsal-javascript-library"></a><span data-ttu-id="6c5c3-350">建立驗證的自訂版本 MSAL JavaScript 程式庫</span><span class="sxs-lookup"><span data-stu-id="6c5c3-350">Build a custom version of the Authentication.MSAL JavaScript library</span></span>

<span data-ttu-id="6c5c3-351">如果應用程式需要 [適用于 JavaScript 的 Microsoft 驗證程式庫自訂版本 ( # A0) ](https://www.npmjs.com/package/@azure/msal-browser)，請執行下列步驟：</span><span class="sxs-lookup"><span data-stu-id="6c5c3-351">If an app requires a custom version of the [Microsoft Authentication Library for JavaScript (MSAL.js)](https://www.npmjs.com/package/@azure/msal-browser), perform the following steps:</span></span>

1. <span data-ttu-id="6c5c3-352">確認系統具有最新的開發人員 .NET SDK，或從 [.NET Core SDK：安裝程式和二進位](https://github.com/dotnet/installer#installers-and-binaries)檔取得並安裝最新的開發人員 SDK。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-352">Confirm the the system has the latest developer .NET SDK or obtain and install the latest developer SDK from [.NET Core SDK: Installers and Binaries](https://github.com/dotnet/installer#installers-and-binaries).</span></span> <span data-ttu-id="6c5c3-353">此案例不需要設定內部 NuGet 摘要。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-353">Configuration of internal NuGet feeds isn't required for this scenario.</span></span>
1. <span data-ttu-id="6c5c3-354">設定 GitHub 存放 `dotnet/aspnetcore` 庫，以根據 [組建 ASP.NET Core 來源](https://github.com/dotnet/aspnetcore/blob/main/docs/BuildFromSource.md)的檔進行開發。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-354">Set up the `dotnet/aspnetcore` GitHub repository for development per the docs at [Build ASP.NET Core from Source](https://github.com/dotnet/aspnetcore/blob/main/docs/BuildFromSource.md).</span></span> <span data-ttu-id="6c5c3-355">分叉和複製或下載 [dotnet/Aspnetcore GitHub 存放庫](https://github.com/dotnet/aspnetcore)的 ZIP 封存。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-355">Fork and clone or download a ZIP archive of the [dotnet/aspnetcore GitHub repository](https://github.com/dotnet/aspnetcore).</span></span>
1. <span data-ttu-id="6c5c3-356">開啟檔案 `src/Components/WebAssembly/Authentication.Msal/src/Interop/package.json` ，並設定所需的版本 `@azure/msal-browser` 。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-356">Open the the `src/Components/WebAssembly/Authentication.Msal/src/Interop/package.json` file and set the desired version of `@azure/msal-browser`.</span></span> <span data-ttu-id="6c5c3-357">如需已發行的版本清單，請造訪 [ `@azure/msal-browser` npm 網站](https://www.npmjs.com/package/@azure/msal-browser)，然後選取 [**版本**] 索引標籤。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-357">For a list of released versions, visit the [`@azure/msal-browser` npm website](https://www.npmjs.com/package/@azure/msal-browser) and select the **Versions** tab.</span></span>
1. <span data-ttu-id="6c5c3-358">`Authentication.Msal` `src/Components/WebAssembly/Authentication.Msal/src` 使用命令 shell 中的命令，在資料夾中建立專案 `yarn build` 。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-358">Build the `Authentication.Msal` project in the `src/Components/WebAssembly/Authentication.Msal/src` folder with the `yarn build` command in a command shell.</span></span>
1. <span data-ttu-id="6c5c3-359">如果應用程式使用 [壓縮的資產 (Brotli/Gzip) ](xref:blazor/host-and-deploy/webassembly#compression)，請壓縮 `Interop/dist/Release/AuthenticationService.js` 檔案。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-359">If the app uses [compressed assets (Brotli/Gzip)](xref:blazor/host-and-deploy/webassembly#compression), compress the `Interop/dist/Release/AuthenticationService.js` file.</span></span>
1. <span data-ttu-id="6c5c3-360">`AuthenticationService.js`從資料夾將檔案和壓縮版本 (`.br` / `.gz`) 複製到應用 `Interop/dist/Release` 程式的 `publish/wwwroot/_content/Microsoft.Authentication.WebAssembly.Msal` 已發佈資產中的應用程式資料夾。</span><span class="sxs-lookup"><span data-stu-id="6c5c3-360">Copy the `AuthenticationService.js` file and compressed versions (`.br`/`.gz`) of the file, if produced, from the `Interop/dist/Release` folder into the app's `publish/wwwroot/_content/Microsoft.Authentication.WebAssembly.Msal` folder in the app's published assets.</span></span>

## <a name="additional-resources"></a><span data-ttu-id="6c5c3-361">其他資源</span><span class="sxs-lookup"><span data-stu-id="6c5c3-361">Additional resources</span></span>

* <xref:blazor/security/webassembly/graph-api>
* [<span data-ttu-id="6c5c3-362">`HttpClient` 以及 `HttpRequestMessage` 使用 FETCH API 要求選項</span><span class="sxs-lookup"><span data-stu-id="6c5c3-362">`HttpClient` and `HttpRequestMessage` with Fetch API request options</span></span>](xref:blazor/call-web-api#httpclient-and-httprequestmessage-with-fetch-api-request-options)
