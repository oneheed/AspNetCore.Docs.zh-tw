...
# <a name="visual-studio"></a>[<span data-ttu-id="52e77-149">Visual Studio</span><span class="sxs-lookup"><span data-stu-id="52e77-149">Visual Studio</span></span>](#tab/visual-studio)
...
* <span data-ttu-id="52e77-162">執行應用程式並選取 **課程** 連結。</span><span class="sxs-lookup"><span data-stu-id="52e77-162">Run the app and select the **Courses** link.</span></span> <span data-ttu-id="52e77-163">部門資料行便會顯示沒有用的 `DepartmentID`。</span><span class="sxs-lookup"><span data-stu-id="52e77-163">The department column displays the `DepartmentID`, which isn't useful.</span></span>
...
<span data-ttu-id="52e77-222">使用下列程式碼更新 *Pages/講師/Index. .cs* ：</span><span class="sxs-lookup"><span data-stu-id="52e77-222">Update *Pages/Instructors/Index.cshtml.cs* with the following code:</span></span>
...
<span data-ttu-id="52e77-262">選取課程，以查看已註冊學生和其年級的清單。</span><span class="sxs-lookup"><span data-stu-id="52e77-262">Select a course to see the list of enrolled students and their grades.</span></span>
...
<span data-ttu-id="52e77-364">`CourseViewModel`：</span><span class="sxs-lookup"><span data-stu-id="52e77-364">The `CourseViewModel`:</span></span>
...
### <a name="create-a-view-model-for-the-instructor-index-view"></a><span data-ttu-id="52e77-383">建立 Instructor [索引] 檢視的檢視模型</span><span class="sxs-lookup"><span data-stu-id="52e77-383">Create a view model for the Instructor Index view</span></span>
...
<span data-ttu-id="52e77-394">以下列程式碼取代 *Pages/Instructors/Index.cshtml.cs*：</span><span class="sxs-lookup"><span data-stu-id="52e77-394">Replace *Pages/Instructors/Index.cshtml.cs* with the following code:</span></span>
...
<span data-ttu-id="52e77-419">按一下 **選取** 連結。</span><span class="sxs-lookup"><span data-stu-id="52e77-419">Click on the **Select** link.</span></span> <span data-ttu-id="52e77-420">資料列樣式變更。</span><span class="sxs-lookup"><span data-stu-id="52e77-420">The row style changes.</span></span>
...
* <span data-ttu-id="52e77-439">造成例外狀況 (由於嘗試在 Null 參考上尋找 `Courses` 屬性)。</span><span class="sxs-lookup"><span data-stu-id="52e77-439">Results in an exception (from trying to find a `Courses` property on a null reference).</span></span>
...
<span data-ttu-id="52e77-451">上述標記會顯示註冊所選取課程的學生清單。</span><span class="sxs-lookup"><span data-stu-id="52e77-451">The preceding markup displays a list of the students who are enrolled in the selected course.</span></span>

<span data-ttu-id="52e77-452">重新整理頁面，然後選取講師。</span><span class="sxs-lookup"><span data-stu-id="52e77-452">Refresh the page and select an instructor.</span></span> <span data-ttu-id="52e77-453">選取課程，以查看已註冊學生和其年級的清單。</span><span class="sxs-lookup"><span data-stu-id="52e77-453">Select a course to see the list of enrolled students and their grades.</span></span>

![Instructors [索引] 頁面已選取講師和課程](read-related-data/_static/instructors-index.png)

## <a name="using-single"></a><span data-ttu-id="52e77-455">使用 Single</span><span class="sxs-lookup"><span data-stu-id="52e77-455">Using Single</span></span>

<span data-ttu-id="52e77-456">`Single` 方法可以傳入 `Where` 條件，而不是個別呼叫 `Where` 方法：</span><span class="sxs-lookup"><span data-stu-id="52e77-456">The `Single` method can pass in the `Where` condition instead of calling the `Where` method separately:</span></span>

[!code-csharp[](intro/samples/cu/Pages/Instructors/IndexSingle.cshtml.cs?name=snippet_single&highlight=21-22,30-31)]

<span data-ttu-id="52e77-457">比起使用 `Where`，上述 `Single` 方法並沒有任何優勢。</span><span class="sxs-lookup"><span data-stu-id="52e77-457">The preceding `Single` approach provides no benefits over using `Where`.</span></span> <span data-ttu-id="52e77-458">某些開發人員偏好使用 `Single` 方法樣式。</span><span class="sxs-lookup"><span data-stu-id="52e77-458">Some developers prefer the `Single` approach style.</span></span>

## <a name="explicit-loading"></a><span data-ttu-id="52e77-459">明確式載入</span><span class="sxs-lookup"><span data-stu-id="52e77-459">Explicit loading</span></span>

<span data-ttu-id="52e77-460">目前程式碼針對 `Enrollments` 和 `Students` 指定積極式載入：</span><span class="sxs-lookup"><span data-stu-id="52e77-460">The current code specifies eager loading for `Enrollments` and `Students`:</span></span>

[!code-csharp[](intro/samples/cu/Pages/Instructors/Index.cshtml.cs?name=snippet_ThenInclude&highlight=6-9)]

<span data-ttu-id="52e77-461">假設使用者很少會想要查看課程中的註冊項目。</span><span class="sxs-lookup"><span data-stu-id="52e77-461">Suppose users rarely want to see enrollments in a course.</span></span> <span data-ttu-id="52e77-462">在此情況下，最佳方法就是在要求時，只載入註冊資料。</span><span class="sxs-lookup"><span data-stu-id="52e77-462">In that case, an optimization would be to only load the enrollment data if it's requested.</span></span> <span data-ttu-id="52e77-463">在本節中，`OnGetAsync` 更新為針對 `Enrollments` 和 `Students` 使用明確式載入。</span><span class="sxs-lookup"><span data-stu-id="52e77-463">In this section, the `OnGetAsync` is updated to use explicit loading of `Enrollments` and `Students`.</span></span>

<span data-ttu-id="52e77-464">使用下列程式碼更新 `OnGetAsync`：</span><span class="sxs-lookup"><span data-stu-id="52e77-464">Update the `OnGetAsync` with the following code:</span></span>

[!code-csharp[](intro/samples/cu/Pages/Instructors/IndexXp.cshtml.cs?name=snippet_OnGetAsync&highlight=9-13,29-35)]

<span data-ttu-id="52e77-465">上述程式碼會捨棄註冊和學生資料的 *ThenInclude* 方法呼叫。</span><span class="sxs-lookup"><span data-stu-id="52e77-465">The preceding code drops the *ThenInclude* method calls for enrollment and student data.</span></span> <span data-ttu-id="52e77-466">如果選取了課程，醒目提示的程式碼就會擷取：</span><span class="sxs-lookup"><span data-stu-id="52e77-466">If a course is selected, the highlighted code retrieves:</span></span>

* <span data-ttu-id="52e77-467">所選取課程的 `Enrollment` 實體。</span><span class="sxs-lookup"><span data-stu-id="52e77-467">The `Enrollment` entities for the selected course.</span></span>
* <span data-ttu-id="52e77-468">每個 `Enrollment` 的 `Student` 實體。</span><span class="sxs-lookup"><span data-stu-id="52e77-468">The `Student` entities for each `Enrollment`.</span></span>

<span data-ttu-id="52e77-469">請注意，上述程式碼會將 `.AsNoTracking()` 註解化。</span><span class="sxs-lookup"><span data-stu-id="52e77-469">Notice the preceding code comments out `.AsNoTracking()`.</span></span> <span data-ttu-id="52e77-470">針對所追蹤的實體，導覽屬性只能進行明確式載入。</span><span class="sxs-lookup"><span data-stu-id="52e77-470">Navigation properties can only be explicitly loaded for tracked entities.</span></span>

<span data-ttu-id="52e77-471">測試應用程式。</span><span class="sxs-lookup"><span data-stu-id="52e77-471">Test the app.</span></span> <span data-ttu-id="52e77-472">就使用者的觀點而言，應用程式行為與之前的版本相同。</span><span class="sxs-lookup"><span data-stu-id="52e77-472">From a users perspective, the app behaves identically to the previous version.</span></span>

<span data-ttu-id="52e77-473">下一個教學課程會示範如何更新相關資料。</span><span class="sxs-lookup"><span data-stu-id="52e77-473">The next tutorial shows how to update related data.</span></span>

## <a name="additional-resources"></a><span data-ttu-id="52e77-474">其他資源</span><span class="sxs-lookup"><span data-stu-id="52e77-474">Additional resources</span></span>

* [<span data-ttu-id="52e77-475">這個教學課程的 YouTube 版本 (第 1 部分)</span><span class="sxs-lookup"><span data-stu-id="52e77-475">YouTube version of this tutorial (part1)</span></span>](https://www.youtube.com/watch?v=PzKimUDmrvE)
* [<span data-ttu-id="52e77-476">這個教學課程的 YouTube 版本 (第 2 部分)</span><span class="sxs-lookup"><span data-stu-id="52e77-476">YouTube version of this tutorial (part2)</span></span>](https://www.youtube.com/watch?v=xvDDrIHv5ko)

>[!div class="step-by-step"]
><span data-ttu-id="52e77-477">[上一個](xref:data/ef-rp/complex-data-model) 
>[下一步](xref:data/ef-rp/update-related-data)</span><span class="sxs-lookup"><span data-stu-id="52e77-477">[Previous](xref:data/ef-rp/complex-data-model)
