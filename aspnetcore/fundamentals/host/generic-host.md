...
依 [預設](xref:fundamentals/configuration/index#default)， *appsettings.json* 和 *appsettings. {環境}。* 當檔案變更時，會重載 json。 若要在 ASP.NET Core 5.0 Preview 3 或更新版本中停用此重載行為，請將索引 `hostBuilder:reloadConfigOnChange` 鍵設定為 `false` 。
