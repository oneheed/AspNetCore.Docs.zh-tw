...
* [Kestrel 伺服器](xref:fundamentals/servers/kestrel)是預設、跨平台的 HTTP 伺服器實作。 Kestrel 可提供最佳的效能和記憶體使用率，但 HTTP.sys 中並沒有一些先進的功能。 如需詳細資訊，請參閱下一節中的 [Kestrel 與 HTTP.sys](#korh) 。
