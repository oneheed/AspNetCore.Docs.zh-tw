...
#### <a name="bearer-token-with-grpc-client-factory"></a>使用 gRPC 用戶端 factory 的持有人權杖

gRPC 用戶端 factory 可以建立使用傳送持有人權杖的用戶端 `ChannelCredentials` 。 設定用戶端時，請指派 `CallCredentials` 用戶端應該與方法搭配使用 `ConfigureChannel` 。
