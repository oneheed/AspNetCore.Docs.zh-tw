...
* 針對下列封裝移除** &lt; DotNetCliToolReference &gt; **元素的參考。 這些工具預設會在 .NET Core CLI 中配套，而不需要個別安裝。
  ...
  *  () 的 SqlConfig 工具 `dotnet sql-cache`
  *  () 的 Microsoft.extensions.secretmanager.tools 工具 `dotnet user-secrets`
...
* .NET Core 2.1.300 SDK (首先包含在 Visual Studio 15.6 中) 將的隱含版本設定 `Microsoft.AspNetCore.App` 為2.1.0，這會造成與 Entity Framework Core 2.1.1 的衝突。 建議的解決方法是將 .NET Core SDK 升級為2.1.301 或更新版本。 如需詳細資訊，請參閱[與 Microsoft 共用相依性的套件 AspNetCore。應用程式無法參考修補程式版本](https://github.com/aspnet/Universe/issues/1180)。
...
* [HTTP Strict 傳輸安全性通訊協定 (的 HSTS) ](xref:security/enforcing-ssl#http-strict-transport-security-protocol-hsts) `UseHsts` 。
...
ASP.NET Core 2.1 提供[ASP.NET Core Identity ](xref:security/authentication/identity) (RCL) 的[ Razor 類別庫](xref:razor-pages/ui-class)。
...
### <a name="update-no-locidentity-to-version-21"></a>更新 Identity 至2.1 版
...
### <a name="replace-no-locidentity-20-ui-with-the-no-locidentity-21-no-locrazor-class-library"></a>Identity以 Identity 2.1 Razor 類別庫取代 2.0 UI
...
### <a name="update-after-scaffolding-no-locidentity"></a>在樣板之後更新Identity
...
## <a name="changes-to-no-locrazor-pages-projects-no-locrazor-files"></a>變更 Razor 頁面專案檔 Razor
