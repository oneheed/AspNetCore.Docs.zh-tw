...
*本節中的指導方針適用于這兩個 Blazor 裝載模型。本節後面的各節提供裝載模型和應用程式類型的其他相關指引。將所有相關區段中的 guidnace 套用至您的應用程式。*
...
1. 在檔案中，將專案 `Shared/MainLayout.razor` 中的所有內容以下列專案括住 `<div>` `class` `page` ：
...
1. 在中 `wwwroot/index.html` ，變更載入元件的元素 `App` ：
