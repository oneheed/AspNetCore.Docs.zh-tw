...
## <a name="changes-to-blazor-app-routing-logic-in-501-and-further-5x-releases-up-to-60"></a>Blazor5.0.1 中的應用程式路由邏輯變更，以及進一步的5.x 版（最高6.0）
...
>
> 將設定 `PreferExactMatches` 為或保持未設定狀態的功能 `@false` *，只是為了回溯相容性而提供*。
>
> 當 .NET 6 發行時，路由器一律會偏好完全相符，而且無法 `PreferExactMatches` 使用選項。

## <a name="update-blazor-webassembly-and-blazor-server-projects"></a>更新 Blazor WebAssembly 和 Blazor Server 專案
...
1. 在 `Program.Main` (`Program.cs`) 中，藉 `<app>` 由在其中新增雜湊來將專案的參考變更為 CSS 選取器 `#` ：
...
1. 在 `Program.Main` (`Program.cs`) 中，將預設的暫時性註冊變更為已設定 `HttpClient` 範圍的範圍（如果有的話）：
...
1. 在 `Program.Main` (的 `Program.cs` **`Client`** 託管解決方案應用程式) 中 Blazor ：
    
   * （選擇性）替代 `builder.HostEnvironment.BaseAddress` 字串用戶端基底位址。
   * 將任何已命名的暫時性用戶端 factory 註冊變更為限域。
...
### <a name="standalone-blazor-webassembly-app-with-microsoft-accounts"></a>Blazor WebAssembly使用 Microsoft 帳戶的獨立應用程式
...
### <a name="standalone-blazor-webassembly-app-with-azure-active-directory-aad"></a>Blazor WebAssembly具有 Azure Active Directory (AAD) 的獨立應用程式
...
### <a name="standalone-blazor-webassembly-app-with-azure-active-directory-aad-b2c"></a>Blazor WebAssembly使用 Azure Active Directory (AAD) B2C 的獨立應用程式
...
### <a name="hosted-blazor-webassembly-app-with-azure-active-directory-aad-or-aad-b2c"></a>Blazor WebAssembly具有 Azure Active Directory (AAD) 或 AAD B2C 的託管應用程式
...
### <a name="update-the-server-project-of-a-hosted-blazor-solution"></a>更新主控方案的伺服器專案 Blazor
...
## <a name="update-a-blazor-progressive-web-application-pwa"></a>更新 Blazor (PWA 的漸進式 Web 應用程式) 
...
## <a name="update-razor-class-libraries-rcls"></a>更新 Razor 類別庫 (RCLs) 
...
## <a name="model-binding-changes-in-aspnet-core-mvc-and-razor-pages"></a>ASP.NET Core MVC 和頁面中的模型系結變更 Razor
