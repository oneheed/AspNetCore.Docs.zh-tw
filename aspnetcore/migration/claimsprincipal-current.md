...
* **HttpCoNtext**。 可存取目前 `HttpContext` (中介軟體的元件（例如) ）可以從 HttpCoNtext 取得目前使用者 `ClaimsPrincipal` 的[HttpContext.User](/dotnet/api/microsoft.aspnetcore.http.httpcontext.user)。
