...
1. 流覽至 PMC 中的專案根目錄，然後執行 [ (EF) Core](/ef/core) `Update-Database` 命令 Entity Framework。
    ...
    EF Core 命令會使用中指定之資料庫的連接字串 *appsettings.json* 。 下列連接字串會以 *localhost* 上的資料庫為目標，名為 *asp-net-core 身分識別* 。 在此設定中，EF Core 設定為使用 `DefaultConnection` 連接字串。
...
1. 選取 [ **View**  >  **SQL Server 物件總管** ]。 展開對應至屬性中所指定之資料庫名稱的節點 `ConnectionStrings:DefaultConnection` *appsettings.json* 。
