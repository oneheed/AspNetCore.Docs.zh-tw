...
*Razor* 標記是以 `@` 符號開頭。 將 c # 程式碼放在以大括弧括住的程式[ Razor 代碼區塊](xref:mvc/views/razor#razor-code-blocks)中， () 來執行 c # 語句 `{ ... }` 。 例如，請參閱上方將 "About" 指派給 `ViewData["Title"]`。 只要使用 `@` 符號參考值，就可以在 HTML 內顯示值。 請參閱上方 `<h2>` 和 `<h3>` 項目的內容。
