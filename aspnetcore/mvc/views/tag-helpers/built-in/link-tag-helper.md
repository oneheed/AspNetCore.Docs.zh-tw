...
<span data-ttu-id="30ea4-109">下列 :::no-loc(Razor)::: 標記顯示 `head` 使用 ASP.NET Core web 應用程式範本所建立的版面配置檔案元素：</span><span class="sxs-lookup"><span data-stu-id="30ea4-109">The following :::no-loc(Razor)::: markup shows the `head` element of a layout file created with the ASP.NET Core web app template:</span></span>
