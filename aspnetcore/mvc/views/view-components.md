...
[查看或下載範例程式碼](https://github.com/dotnet/AspNetCore.Docs/tree/main/aspnetcore/mvc/views/view-components/sample) ([如何下載](xref:index#how-to-download-a-sample)) 
...
當您考慮 view 元件是否符合應用程式的規格時，請考慮 Razor 改用 components。 Razor 元件也會結合標記與 c # 程式碼，以產生可重複使用的 UI 單位。 Razor 元件是為開發人員提供用戶端 UI 邏輯和組合時的生產力而設計。 如需詳細資訊，請參閱<xref:blazor/components/index>。 如需有關如何將 Razor 元件併入 MVC 或 Razor Pages 應用程式的詳細資訊，請參閱 <xref:blazor/components/prerendering-and-integration?pivots=server> 。
...
若要自訂視圖搜尋路徑，請修改 Razor 的 <xref:Microsoft.AspNetCore.Mvc.Razor.RazorViewEngineOptions.ViewLocationFormats> 集合。 例如，若要在路徑 "/Components/{View Component Name}/{檢視 Name}" 內搜尋視圖，請將新的專案加入至集合：
...
[下載](https://github.com/dotnet/AspNetCore.Docs/tree/main/aspnetcore/mvc/views/view-components/sample)、建置和測試起始程式碼。 它是具有 `ToDo` 控制器的簡單專案，而此控制器顯示 *ToDO* 項目清單。
...
### <a name="create-the-view-component-razor-view"></a>建立 view 元件 Razor 視圖
...
* 建立 *Views/Shared/Components/prioritylist default.cshtml/Default。 cshtml* Razor view：
...
將 `using` 語句新增至您 Razor 的 view 檔案，然後使用 `nameof` 運算子：
...
在標記檔案中使用 view component Tag Helper Razor ：
