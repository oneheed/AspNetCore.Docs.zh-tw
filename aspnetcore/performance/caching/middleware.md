...
回應快取中介軟體只會快取會導致 200 (確定) 狀態碼的伺服器回應。 中介軟體會忽略任何其他回應，包括[錯誤頁面](xref:fundamentals/error-handling)。
...
| `Set-Cookie` | 如果標頭存在，則不會快取回應。 在要求處理管線中設定一或多個的中介軟體， cookie 會防止回應快取中介軟體快取回應 (例如，以為[ cookie 基礎的 TempData 提供者](xref:fundamentals/app-state#tempdata)) 。  |
...
| `If-None-Match` | 如果值不是 `*` ，而且 `ETag` 回應的不符合任何提供的值，則會從快取中提供完整回應。 否則，會提供 304 (未修改) 回應。 |
...
* `Set-Cookie`標頭不得存在。
* `Vary`標頭參數必須有效，而且不等於 `*` 。
* `Content-Length`如果設定) 必須符合回應主體的大小，則標頭值 (。
...
> [!NOTE]
> 用來產生安全權杖以防止跨網站偽造要求的 Antiforgery 系統 (CSRF) 攻擊會將 `Cache-Control` 和 `Pragma` 標頭設定為， `no-cache` 如此就不會快取回應。 如需如何停用 HTML 表單專案之 antiforgery token 的詳細資訊，請參閱 <xref:security/anti-request-forgery#aspnet-core-antiforgery-configuration> 。
