---
title: '在 ASP.NET Core 專案中新增、下載及刪除使用者資料 :::no-loc(Identity):::'
author: rick-anderson
description: '瞭解如何將自訂使用者資料新增至 :::no-loc(Identity)::: ASP.NET Core 專案。 刪除每個 GDPR 的資料。'
ms.author: riande
ms.date: 03/26/2020
...
---
# <a name="add-download-and-delete-custom-user-data-to-no-locidentity-in-an-aspnet-core-project"></a><span data-ttu-id="d08f4-104">在 ASP.NET Core 專案中新增、下載及刪除自訂使用者資料 :::no-loc(Identity):::</span><span class="sxs-lookup"><span data-stu-id="d08f4-104">Add, download, and delete custom user data to :::no-loc(Identity)::: in an ASP.NET Core project</span></span>
...
* <span data-ttu-id="d08f4-108">使用屬性標記自訂使用者資料模型 <xref:Microsoft.AspNetCore.:::no-loc(Identity):::.PersonalDataAttribute> ，讓它自動可供下載及刪除。</span><span class="sxs-lookup"><span data-stu-id="d08f4-108">Mark the custom user data model with the <xref:Microsoft.AspNetCore.:::no-loc(Identity):::.PersonalDataAttribute> attribute so it's automatically available for download and deletion.</span></span> <span data-ttu-id="d08f4-109">讓資料能夠下載及刪除，有助於符合 [GDPR](xref:security/gdpr) 需求。</span><span class="sxs-lookup"><span data-stu-id="d08f4-109">Making the data able to be downloaded and deleted helps meet [GDPR](xref:security/gdpr) requirements.</span></span>

<span data-ttu-id="d08f4-110">專案範例是從 :::no-loc(Razor)::: 頁面 web 應用程式建立的，但是 ASP.NET CORE MVC web 應用程式的相關指示也很類似。</span><span class="sxs-lookup"><span data-stu-id="d08f4-110">The project sample is created from a :::no-loc(Razor)::: Pages web app, but the instructions are similar for a ASP.NET Core MVC web app.</span></span>
...
## <a name="create-a-no-locrazor-web-app"></a><span data-ttu-id="d08f4-113">建立 :::no-loc(Razor)::: web 應用程式</span><span class="sxs-lookup"><span data-stu-id="d08f4-113">Create a :::no-loc(Razor)::: web app</span></span>
...
## <a name="run-the-no-locidentity-scaffolder"></a><span data-ttu-id="d08f4-128">執行 :::no-loc(Identity)::: scaffolder</span><span class="sxs-lookup"><span data-stu-id="d08f4-128">Run the :::no-loc(Identity)::: scaffolder</span></span>
...
* <span data-ttu-id="d08f4-131">從 [ **新增 Scaffold** ] 對話方塊的左窗格中，選取 [ **:::no-loc(Identity):::**  >  **新增** ]。</span><span class="sxs-lookup"><span data-stu-id="d08f4-131">From the left pane of the **Add Scaffold** dialog, select **:::no-loc(Identity):::** > **Add** .</span></span>
* <span data-ttu-id="d08f4-132">在 [ **新增 :::no-loc(Identity):::** ] 對話方塊中，有下列選項：</span><span class="sxs-lookup"><span data-stu-id="d08f4-132">In the **Add :::no-loc(Identity):::** dialog, the following options:</span></span>
...
<span data-ttu-id="d08f4-146">執行下列命令以列出 :::no-loc(Identity)::: scaffolder 選項：</span><span class="sxs-lookup"><span data-stu-id="d08f4-146">Run the following command to list the :::no-loc(Identity)::: scaffolder options:</span></span>

```dotnetcli
dotnet aspnet-codegenerator identity -h
```

<span data-ttu-id="d08f4-147">在專案資料夾中，執行 :::no-loc(Identity)::: scaffolder：</span><span class="sxs-lookup"><span data-stu-id="d08f4-147">In the project folder, run the :::no-loc(Identity)::: scaffolder:</span></span>
...
## <a name="add-custom-user-data-to-the-no-locidentity-db"></a><span data-ttu-id="d08f4-159">將自訂使用者資料新增至 :::no-loc(Identity)::: 資料庫</span><span class="sxs-lookup"><span data-stu-id="d08f4-159">Add custom user data to the :::no-loc(Identity)::: DB</span></span>

<span data-ttu-id="d08f4-160">`:::no-loc(Identity):::User`使用自訂屬性更新衍生的類別。</span><span class="sxs-lookup"><span data-stu-id="d08f4-160">Update the `:::no-loc(Identity):::User` derived class with custom properties.</span></span> <span data-ttu-id="d08f4-161">如果您將專案命名為 WebApp1，該檔案就會命名為 *Areas/ :::no-loc(Identity)::: /Data/WebApp1User.cs* 。</span><span class="sxs-lookup"><span data-stu-id="d08f4-161">If you named the project WebApp1, the file is named *Areas/:::no-loc(Identity):::/Data/WebApp1User.cs* .</span></span> <span data-ttu-id="d08f4-162">以下列程式碼更新檔案：</span><span class="sxs-lookup"><span data-stu-id="d08f4-162">Update the file with the following code:</span></span>
...
* <span data-ttu-id="d08f4-164">在 *區域/ :::no-loc(Identity)::: /Pages/Account/Manage/DeletePersonalData.cshtml* :::no-loc(Razor)::: 頁面呼叫時刪除 `UserManager.Delete` 。</span><span class="sxs-lookup"><span data-stu-id="d08f4-164">Deleted when the *Areas/:::no-loc(Identity):::/Pages/Account/Manage/DeletePersonalData.cshtml* :::no-loc(Razor)::: Page calls `UserManager.Delete`.</span></span>
* <span data-ttu-id="d08f4-165">由 *區域/ :::no-loc(Identity)::: /Pages/Account/Manage/DownloadPersonalData.cshtml* 頁面包含在下載的資料中 :::no-loc(Razor)::: 。</span><span class="sxs-lookup"><span data-stu-id="d08f4-165">Included in the downloaded data by the *Areas/:::no-loc(Identity):::/Pages/Account/Manage/DownloadPersonalData.cshtml* :::no-loc(Razor)::: Page.</span></span>
...
<span data-ttu-id="d08f4-167">以下列醒目提示的程式 `InputModel` 代碼更新 *區域/ :::no-loc(Identity)::: /Pages/Account/Manage/Index.cshtml.cs* 中的：</span><span class="sxs-lookup"><span data-stu-id="d08f4-167">Update the `InputModel` in *Areas/:::no-loc(Identity):::/Pages/Account/Manage/Index.cshtml.cs* with the following highlighted code:</span></span>
...
<span data-ttu-id="d08f4-168">以下列醒目提示的標記更新 *區域/ :::no-loc(Identity)::: /Pages/Account/Manage/Index.cshtml* ：</span><span class="sxs-lookup"><span data-stu-id="d08f4-168">Update the *Areas/:::no-loc(Identity):::/Pages/Account/Manage/Index.cshtml* with the following highlighted markup:</span></span>
...
<span data-ttu-id="d08f4-169">以下列醒目提示的標記更新 *區域/ :::no-loc(Identity)::: /Pages/Account/Manage/Index.cshtml* ：</span><span class="sxs-lookup"><span data-stu-id="d08f4-169">Update the *Areas/:::no-loc(Identity):::/Pages/Account/Manage/Index.cshtml* with the following highlighted markup:</span></span>
...
<span data-ttu-id="d08f4-171">以下列醒目提示的程式 `InputModel` 代碼更新 *區域/ :::no-loc(Identity)::: /Pages/Account/Register.cshtml.cs* 中的：</span><span class="sxs-lookup"><span data-stu-id="d08f4-171">Update the `InputModel` in *Areas/:::no-loc(Identity):::/Pages/Account/Register.cshtml.cs* with the following highlighted code:</span></span>
...
<span data-ttu-id="d08f4-172">以下列醒目提示的標記更新 *區域/ :::no-loc(Identity)::: /Pages/Account/Register.cshtml* ：</span><span class="sxs-lookup"><span data-stu-id="d08f4-172">Update the *Areas/:::no-loc(Identity):::/Pages/Account/Register.cshtml* with the following highlighted markup:</span></span>
...
<span data-ttu-id="d08f4-173">以下列醒目提示的標記更新 *區域/ :::no-loc(Identity)::: /Pages/Account/Register.cshtml* ：</span><span class="sxs-lookup"><span data-stu-id="d08f4-173">Update the *Areas/:::no-loc(Identity):::/Pages/Account/Register.cshtml* with the following highlighted markup:</span></span>
...
* <span data-ttu-id="d08f4-182">查看頁面上的自訂使用者資料 `/:::no-loc(Identity):::/Account/Manage` 。</span><span class="sxs-lookup"><span data-stu-id="d08f4-182">View the custom user data on the `/:::no-loc(Identity):::/Account/Manage` page.</span></span>
* <span data-ttu-id="d08f4-183">從頁面下載並查看使用者的個人資料 `/:::no-loc(Identity):::/Account/Manage/PersonalData` 。</span><span class="sxs-lookup"><span data-stu-id="d08f4-183">Download and view the users personal data from the `/:::no-loc(Identity):::/Account/Manage/PersonalData` page.</span></span>

## <a name="add-claims-to-no-locidentity-using-iuserclaimsprincipalfactoryapplicationuser"></a><span data-ttu-id="d08f4-184">:::no-loc(Identity):::使用 IUserClaimsPrincipalFactory 新增宣告<ApplicationUser></span><span class="sxs-lookup"><span data-stu-id="d08f4-184">Add claims to :::no-loc(Identity)::: using IUserClaimsPrincipalFactory<ApplicationUser></span></span>
...
<span data-ttu-id="d08f4-187">您可以使用介面將其他宣告加入至 :::no-loc(ASP.NET Core Identity)::: `IUserClaimsPrincipalFactory<T>` 。</span><span class="sxs-lookup"><span data-stu-id="d08f4-187">Additional claims can be added to :::no-loc(ASP.NET Core Identity)::: by using the `IUserClaimsPrincipalFactory<T>` interface.</span></span> <span data-ttu-id="d08f4-188">此類別可以新增至方法中的應用程式 `Startup.ConfigureServices` 。</span><span class="sxs-lookup"><span data-stu-id="d08f4-188">This class can be added to the app in the `Startup.ConfigureServices` method.</span></span> <span data-ttu-id="d08f4-189">新增類別的自訂執行，如下所示：</span><span class="sxs-lookup"><span data-stu-id="d08f4-189">Add the custom implementation of the class as follows:</span></span>
...
<span data-ttu-id="d08f4-194">然後，您可以在應用程式中使用額外的宣告。</span><span class="sxs-lookup"><span data-stu-id="d08f4-194">The additional claim can then be used in the app.</span></span> <span data-ttu-id="d08f4-195">在 :::no-loc(Razor)::: 頁面中， `IAuthorizationService` 實例可以用來存取宣告值。</span><span class="sxs-lookup"><span data-stu-id="d08f4-195">In a :::no-loc(Razor)::: Page, the `IAuthorizationService` instance can be used to access the claim value.</span></span>
