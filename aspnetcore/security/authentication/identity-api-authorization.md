...
ASP.NET Core 3.0 或更新版本在單一頁面應用程式中提供驗證， (使用 API 授權支援的 Spa) 。 Identity用於驗證和儲存使用者的 ASP.NET Core 會與[ Identity 伺服器](https://identityserver.io/)結合，以執行 OpenID connect。

驗證參數已加入至「**角度**」和「**回應**」專案範本，類似于 Web 應用程式中的驗證參數** (模型-視圖控制器) ** (MVC) 和**web 應用程式** (Razor 頁面) 專案範本。 允許的參數值為 [**無**] 和 [**個人**]。 **React.js 和 Redux**專案範本目前不支援驗證參數。
...
下列程式碼範例會依賴[AspNetCore. ApiAuthorization。 Identity伺服器](https://www.nuget.org/packages/Microsoft.AspNetCore.ApiAuthorization.IdentityServer)NuGet 套件。 這些範例會使用 <xref:Microsoft.Extensions.DependencyInjection.IdentityServerBuilderConfigurationExtensions.AddApiAuthorization%2A> 和擴充方法來設定 API 驗證和授權 <xref:Microsoft.AspNetCore.ApiAuthorization.IdentityServer.ApiResourceCollection.AddIdentityServerJwt%2A> 。 使用 [回應] 或 [角度 SPA] 專案範本搭配驗證的專案，包括對此套件的參考。
...
* 在 `Startup.ConfigureServices` 方法內：
  ...
  * Identity伺服器具有其他 `AddApiAuthorization` helper 方法，可在伺服器上設定一些預設的 ASP.NET Core 慣例 Identity ：
  ...
  * 使用其他 helper 方法進行驗證，以設定 `AddIdentityServerJwt` 應用程式來驗證服務器所產生的 JWT 權杖 Identity ：
...
此 helper 方法會 Identity 將伺服器設定為使用我們支援的設定。 Identity伺服器是一種功能強大且可擴充的架構，可處理應用程式的安全性考慮。 同時，會在最常見的案例中公開不必要的複雜性。 因此，系統會為您提供一組慣例和設定選項，並將其視為良好的起點。 一旦您的驗證需要變更，伺服器的完整功能 Identity 仍可供自訂驗證以符合您的需求。

### <a name="addno-locidentityserverjwt"></a>新增 Identity ServerJwt

此 helper 方法會將應用程式的原則配置設定為預設驗證處理常式。 原則會設定為讓 Identity 處理所有傳送至 Identity URL 空間 "/" 中任何子路徑的要求 Identity 。 會 `JwtBearerHandler` 處理所有其他要求。 此外，此方法會向 `<<ApplicationName>>API` Server 註冊具有 Identity 預設範圍的 API 資源 `<<ApplicationName>>API` ，並設定 JWT 持有人權杖中介軟體，以驗證 Identity 伺服器針對應用程式所簽發的權杖。
...
在*Data\ApplicationDbCoNtext.cs*檔案中，請注意， `DbContext` 中會使用相同的， Identity 因為它會擴充 `ApiAuthorizationDbContext` (更多衍生的類別， `IdentityDbContext`) 以包含伺服器的架構 Identity 。
...
請務必注意，保護路由並不會保護實際的端點 (但仍然需要套用 `[Authorize]` 屬性) 但它只會防止使用者在未驗證時流覽至指定的用戶端路由。

## <a name="authenticate-api-requests-angular"></a> (角度) 驗證 API 要求
...
* 不會保護實際的端點 (但仍然需要將 `[Authorize]` 屬性套用至它) 。
...
API 授權的支援建置於 Identity 具有一組慣例、預設值和增強功能的伺服器之上，以簡化 spa 的體驗。 不用說， Identity 如果 ASP.NET Core 整合不會涵蓋您的案例，伺服器的完整功能就會在幕後提供。 ASP.NET Core 支援著重于「第一方」應用程式，其中的所有應用程式都是由我們的組織建立和部署。 因此，不會針對同意或同盟等專案提供支援。 針對這些案例，請使用 Identity 伺服器並遵循其檔。

### <a name="application-profiles"></a>應用程式佈建檔

應用程式佈建檔是針對進一步定義其參數之應用程式預先定義的設定。 目前支援下列設定檔：

* `IdentityServerSPA`：代表與伺服器一起主控 Identity 的 SPA，做為單一單位。
  ...
  * 允許的回應模式為 `fragment` 。
* `SPA`：代表不是由伺服器主控的 SPA Identity 。
  * 一組範圍包含 `openid` 、 `profile` ，以及為應用程式中的 api 定義的每個範圍。
  * 一組允許的 OIDC 回應類型為 `id_token token` 或個別 (`id_token` ， `token`) 。
  * 允許的回應模式為 `fragment` 。
* `IdentityServerJwt`：代表與伺服器一起裝載的 API Identity 。
  * 應用程式已設定為具有單一範圍，預設為應用程式名稱。
* `API`：代表不是由伺服器裝載的 API Identity 。
  * 應用程式已設定為具有單一範圍，預設為應用程式名稱。
