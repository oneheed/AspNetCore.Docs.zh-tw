...
* 輸入 `/signin-twitter` 附加至 [**回呼 url** ] 欄位的開發 URI， (例如： `https://webapp128.azurewebsites.net/signin-twitter`) 。 稍後在此範例中設定的 Twitter 驗證配置會自動處理 `/signin-twitter` 路由上的要求，以執行 OAuth 流程。
