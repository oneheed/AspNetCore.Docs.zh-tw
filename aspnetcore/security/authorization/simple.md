...
<xref:Microsoft.AspNetCore.Authorization.AuthorizeAttribute>***無法***套用至 Razor 頁面處理常式。 例如， `[Authorize]` 無法套用至 `OnGet` 、 `OnPost` 或任何其他頁面處理常式。 針對不同的處理常式，請考慮針對具有不同授權需求的頁面使用 ASP.NET Core 的 MVC 控制器。

下列兩種方法可以用來將授權套用至 Razor 頁面處理常式方法：

* 針對需要不同授權的頁面處理常式，請使用不同的頁面。 已將共用內容移至一或多個 [部分的視圖](xref:mvc/views/partial)。 可能的話，這是建議的方法。
