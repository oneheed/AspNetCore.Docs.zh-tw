...
<xref:Microsoft.AspNetCore.Authorization.AuthorizeAttribute>可以 * **not** _ 套用至 Razor 頁面處理常式。 例如， `[Authorize]` 無法套用至 `OnGet` 、 `OnPost` 或任何其他頁面處理常式。 針對不同的處理常式，請考慮針對具有不同授權需求的頁面使用 ASP.NET Core 的 MVC 控制器。

下列兩種方法可以用來將授權套用至 Razor 頁面處理常式方法：

_ 針對需要不同授權的頁面處理常式使用不同的頁面。 已將共用內容移至一或多個 [部分的視圖](xref:mvc/views/partial)。 可能的話，這是建議的方法。
...
> [!WARNING]
> [PageHandlerAuth](https://github.com/pranavkm/PageHandlerAuth)範例方法會使用適用于頁面、頁面模型或全域的授權屬性來撰寫 * **not** _： _。 當您有一或多個 `AuthorizeAttribute` 實例也套用至頁面時，撰寫授權屬性會導致驗證和授權執行多次 `AuthorizeFilter` 。
