...
>[!WARNING]
> 請勿在 JavaScript 中串連未受信任的輸入，以建立 DOM 元素。 您應該 `createElement()` 適當地使用和指派屬性值（例如 `node.TextContent=` ）， `element.SetAttribute()` / `element[attribute]=` 否則您會將自己公開給以 DOM 為基礎的 XSS。
