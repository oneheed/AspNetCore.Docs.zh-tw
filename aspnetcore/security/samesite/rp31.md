...
ASP.NET Core 3.0 內建支援 [SameSite](https://www.owasp.org/index.php/SameSite) 屬性，包括的 `SameSiteMode` 屬性值， `Unspecified` 以抑制寫入屬性。
...
* 如果 `SameSite` 設定為 `None` ，且目前的使用者代理程式已知不支援 none 屬性值，則為。 檢查是使用 [SameSiteSupport](https://github.com/dotnet/AspNetCore.Docs/blob/main/aspnetcore/security/samesite/snippets/SameSiteSupport.cs) 類別來完成：
