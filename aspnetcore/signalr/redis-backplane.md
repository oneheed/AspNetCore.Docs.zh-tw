...
---
# <a name="set-up-a-redis-backplane-for-aspnet-core-no-locsignalr-scale-out"></a><span data-ttu-id="91346-103">設定 Redis 背板來 ASP.NET Core SignalR 向外延展</span><span class="sxs-lookup"><span data-stu-id="91346-103">Set up a Redis backplane for ASP.NET Core SignalR scale-out</span></span>
...
<span data-ttu-id="91346-105">本文說明 SignalR 設定 [Redis](https://redis.io/) 伺服器以用於相應放大 ASP.NET Core 應用程式的特定層面 SignalR 。</span><span class="sxs-lookup"><span data-stu-id="91346-105">This article explains SignalR-specific aspects of setting up a [Redis](https://redis.io/) server to use for scaling out an ASP.NET Core SignalR app.</span></span>
...
* <span data-ttu-id="91346-107">部署 Redis 伺服器。</span><span class="sxs-lookup"><span data-stu-id="91346-107">Deploy a Redis server.</span></span>
  ...
  > [!IMPORTANT] 
  > <span data-ttu-id="91346-108">在生產環境中，建議只有在與應用程式在相同的資料中心執行時，才建議使用 Redis 背板 SignalR 。</span><span class="sxs-lookup"><span data-stu-id="91346-108">For production use, a Redis backplane is recommended only when it runs in the same data center as the SignalR app.</span></span> <span data-ttu-id="91346-109">否則，網路延遲會降低效能。</span><span class="sxs-lookup"><span data-stu-id="91346-109">Otherwise, network latency degrades performance.</span></span> <span data-ttu-id="91346-110">如果您 SignalR 的應用程式是在 azure 雲端中執行，我們建議 SignalR 使用 azure 服務，而不是 Redis 背板。</span><span class="sxs-lookup"><span data-stu-id="91346-110">If your SignalR app is running in the Azure cloud, we recommend Azure SignalR Service instead of a Redis backplane.</span></span> <span data-ttu-id="91346-111">您可以針對開發和測試環境使用 Azure Redis 快取服務。</span><span class="sxs-lookup"><span data-stu-id="91346-111">You can use the Azure Redis Cache Service for development and test environments.</span></span>
...
* <span data-ttu-id="91346-115">在 SignalR 應用程式中，安裝 `Microsoft.AspNetCore.SignalR.Redis` NuGet 套件。</span><span class="sxs-lookup"><span data-stu-id="91346-115">In the SignalR app, install the `Microsoft.AspNetCore.SignalR.Redis` NuGet package.</span></span>
* <span data-ttu-id="91346-116">在 `Startup.ConfigureServices` 方法中，在下列情況下呼叫 `AddRedis` `AddSignalR` ：</span><span class="sxs-lookup"><span data-stu-id="91346-116">In the `Startup.ConfigureServices` method, call `AddRedis` after `AddSignalR`:</span></span>
...
* <span data-ttu-id="91346-123">在 SignalR 應用程式中，安裝下列其中一個 NuGet 套件：</span><span class="sxs-lookup"><span data-stu-id="91346-123">In the SignalR app, install one of the following NuGet packages:</span></span>

  * <span data-ttu-id="91346-124">`Microsoft.AspNetCore.SignalR.StackExchangeRedis` -相依于 >stackexchange.redis. Redis 2. X.X。</span><span class="sxs-lookup"><span data-stu-id="91346-124">`Microsoft.AspNetCore.SignalR.StackExchangeRedis` - Depends on StackExchange.Redis 2.X.X.</span></span> <span data-ttu-id="91346-125">這是 ASP.NET Core 2.2 和更新版本的建議套件。</span><span class="sxs-lookup"><span data-stu-id="91346-125">This is the recommended package for ASP.NET Core 2.2 and later.</span></span>
  * <span data-ttu-id="91346-126">`Microsoft.AspNetCore.SignalR.Redis` -相依于 >stackexchange.redis. Redis 1. X.X。</span><span class="sxs-lookup"><span data-stu-id="91346-126">`Microsoft.AspNetCore.SignalR.Redis` - Depends on StackExchange.Redis 1.X.X.</span></span> <span data-ttu-id="91346-127">此套件不包含在 ASP.NET Core 3.0 和更新版本中。</span><span class="sxs-lookup"><span data-stu-id="91346-127">This package isn't included in ASP.NET Core 3.0 and later.</span></span>
...
* <span data-ttu-id="91346-128">在 `Startup.ConfigureServices` 方法中，呼叫 <xref:Microsoft.Extensions.DependencyInjection.StackExchangeRedisDependencyInjectionExtensions.AddStackExchangeRedis*> ：</span><span class="sxs-lookup"><span data-stu-id="91346-128">In the `Startup.ConfigureServices` method, call <xref:Microsoft.Extensions.DependencyInjection.StackExchangeRedisDependencyInjectionExtensions.AddStackExchangeRedis*>:</span></span>
 ...
 <span data-ttu-id="91346-129">使用時 `Microsoft.AspNetCore.SignalR.Redis` ，請呼叫 <xref:Microsoft.Extensions.DependencyInjection.RedisDependencyInjectionExtensions.AddRedis*> 。</span><span class="sxs-lookup"><span data-stu-id="91346-129">When using `Microsoft.AspNetCore.SignalR.Redis`, call <xref:Microsoft.Extensions.DependencyInjection.RedisDependencyInjectionExtensions.AddRedis*>.</span></span>
...
* <span data-ttu-id="91346-130">視需要設定選項：</span><span class="sxs-lookup"><span data-stu-id="91346-130">Configure options as needed:</span></span>
 ...
 <span data-ttu-id="91346-135">使用時 `Microsoft.AspNetCore.SignalR.Redis` ，請呼叫 <xref:Microsoft.Extensions.DependencyInjection.RedisDependencyInjectionExtensions.AddRedis*> 。</span><span class="sxs-lookup"><span data-stu-id="91346-135">When using `Microsoft.AspNetCore.SignalR.Redis`, call <xref:Microsoft.Extensions.DependencyInjection.RedisDependencyInjectionExtensions.AddRedis*>.</span></span>
...
* <span data-ttu-id="91346-138">在 SignalR 應用程式中，安裝下列 NuGet 套件：</span><span class="sxs-lookup"><span data-stu-id="91346-138">In the SignalR app, install the following NuGet package:</span></span>
...
* <span data-ttu-id="91346-147">如果您針對多個應用程式使用一個 Redis 伺服器 SignalR ，請針對每個應用程式使用不同的通道前置詞 SignalR 。</span><span class="sxs-lookup"><span data-stu-id="91346-147">If you're using one Redis server for multiple SignalR apps, use a different channel prefix for each SignalR app.</span></span>

  <span data-ttu-id="91346-148">設定通道首碼會隔離一個 SignalR 應用程式與其他使用不同通道首碼的應用程式。</span><span class="sxs-lookup"><span data-stu-id="91346-148">Setting a channel prefix isolates one SignalR app from others that use different channel prefixes.</span></span> <span data-ttu-id="91346-149">如果您未指派不同的前置詞，則從某個應用程式傳送到其所有用戶端的訊息將會移至所有使用 Redis 伺服器做為背板的應用程式的所有用戶端。</span><span class="sxs-lookup"><span data-stu-id="91346-149">If you don't assign different prefixes, a message sent from one app to all of its own clients will go to all clients of all apps that use the Redis server as a backplane.</span></span>
...
<span data-ttu-id="91346-162">SignalR 當伺服器恢復運作時，不會緩衝處理訊息來傳送訊息。</span><span class="sxs-lookup"><span data-stu-id="91346-162">SignalR doesn't buffer messages to send them when the server comes back up.</span></span> <span data-ttu-id="91346-163">Redis 伺服器關閉時傳送的任何訊息都會遺失。</span><span class="sxs-lookup"><span data-stu-id="91346-163">Any messages sent while the Redis server is down are lost.</span></span>

<span data-ttu-id="91346-164">SignalR 當 Redis 伺服器再次可用時，會自動重新連接。</span><span class="sxs-lookup"><span data-stu-id="91346-164">SignalR automatically reconnects when the Redis server is available again.</span></span>
