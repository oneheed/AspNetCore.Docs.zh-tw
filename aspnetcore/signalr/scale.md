...
---
# <a name="aspnet-core-no-locsignalr-hosting-and-scaling"></a><span data-ttu-id="909fb-103">ASP.NET Core SignalR 裝載和調整</span><span class="sxs-lookup"><span data-stu-id="909fb-103">ASP.NET Core SignalR hosting and scaling</span></span>
...
<span data-ttu-id="909fb-105">本文說明使用 ASP.NET Core 之高流量應用程式的裝載和調整考慮 SignalR 。</span><span class="sxs-lookup"><span data-stu-id="909fb-105">This article explains hosting and scaling considerations for high-traffic apps that use ASP.NET Core SignalR.</span></span>
...
1. <span data-ttu-id="909fb-114">使用 Azure 服務時 SignalR 。</span><span class="sxs-lookup"><span data-stu-id="909fb-114">When using the Azure SignalR Service.</span></span>
...
<span data-ttu-id="909fb-117">如需設定 Azure App Service 的指引 SignalR ，請參閱 <xref:signalr/publish-to-azure-web-app> 。</span><span class="sxs-lookup"><span data-stu-id="909fb-117">For guidance on configuring Azure App Service for SignalR, see <xref:signalr/publish-to-azure-web-app>.</span></span>
...
<span data-ttu-id="909fb-126">大量使用連接相關的資源， SignalR 可能會影響裝載于相同伺服器上的其他 web 應用程式。</span><span class="sxs-lookup"><span data-stu-id="909fb-126">The heavy use of connection-related resources by SignalR can affect other web apps that are hosted on the same server.</span></span> <span data-ttu-id="909fb-127">當 SignalR 開啟並保存最後一個可用的 TCP 連線時，相同伺服器上的其他 web 應用程式也不會有其他可用的連接。</span><span class="sxs-lookup"><span data-stu-id="909fb-127">When SignalR opens and holds the last available TCP connections, other web apps on the same server also have no more connections available to them.</span></span>
...
<span data-ttu-id="909fb-130">若要讓 SignalR 資源使用狀況在其他 web 應用程式中造成錯誤，請 SignalR 在不同于其他 web 應用程式的伺服器上執行。</span><span class="sxs-lookup"><span data-stu-id="909fb-130">To keep SignalR resource usage from causing errors in other web apps, run SignalR on different servers than your other web apps.</span></span>

<span data-ttu-id="909fb-131">若要防止 SignalR 資源使用在應用程式中造成錯誤 SignalR ，請相應放大以限制伺服器必須處理的連接數目。</span><span class="sxs-lookup"><span data-stu-id="909fb-131">To keep SignalR resource usage from causing errors in a SignalR app, scale out to limit the number of connections a server has to handle.</span></span>
...
<span data-ttu-id="909fb-133">使用的應用程式 SignalR 需要追蹤其所有連接，這會造成伺服器陣列的問題。</span><span class="sxs-lookup"><span data-stu-id="909fb-133">An app that uses SignalR needs to keep track of all its connections, which creates problems for a server farm.</span></span> <span data-ttu-id="909fb-134">新增伺服器，並取得其他伺服器不知道的新連接。</span><span class="sxs-lookup"><span data-stu-id="909fb-134">Add a server, and it gets new connections that the other servers don't know about.</span></span> <span data-ttu-id="909fb-135">例如，在 SignalR 下圖中的每部伺服器上，並不知道其他伺服器上的連接。</span><span class="sxs-lookup"><span data-stu-id="909fb-135">For example, SignalR on each server in the following diagram is unaware of the connections on the other servers.</span></span> <span data-ttu-id="909fb-136">當 SignalR 其中一個伺服器想要將訊息傳送給所有用戶端時，訊息只會傳送到連接到該伺服器的用戶端。</span><span class="sxs-lookup"><span data-stu-id="909fb-136">When SignalR on one of the servers wants to send a message to all clients, the message only goes to the clients connected to that server.</span></span>

![調整：：：非 loc (SignalR) ：：：沒有背板](scale/_static/scale-no-backplane.png)

<span data-ttu-id="909fb-138">解決此問題的選項是 [Azure SignalR 服務](#azure-signalr-service) 和 [Redis 背板](#redis-backplane)。</span><span class="sxs-lookup"><span data-stu-id="909fb-138">The options for solving this problem are the [Azure SignalR Service](#azure-signalr-service) and [Redis backplane](#redis-backplane).</span></span>

## <a name="azure-no-locsignalr-service"></a><span data-ttu-id="909fb-139">Azure SignalR 服務</span><span class="sxs-lookup"><span data-stu-id="909fb-139">Azure SignalR Service</span></span>

<span data-ttu-id="909fb-140">Azure SignalR 服務是 proxy，而不是背板。</span><span class="sxs-lookup"><span data-stu-id="909fb-140">The Azure SignalR Service is a proxy rather than a backplane.</span></span> <span data-ttu-id="909fb-141">每次用戶端啟動伺服器的連線時，就會將用戶端重新導向以連接至服務。</span><span class="sxs-lookup"><span data-stu-id="909fb-141">Each time a client initiates a connection to the server, the client is redirected to connect to the service.</span></span> <span data-ttu-id="909fb-142">下圖說明該流程：</span><span class="sxs-lookup"><span data-stu-id="909fb-142">That process is illustrated in the following diagram:</span></span>
...
* <span data-ttu-id="909fb-147">由於用戶端連線時，會立即將用戶端重新導向至 Azure 服務，因此不需要像是 [用戶端親和性](/iis/extensions/configuring-application-request-routing-arr/http-load-balancing-using-application-request-routing#step-3---configure-client-affinity)的「粘滯話」 SignalR 。</span><span class="sxs-lookup"><span data-stu-id="909fb-147">Sticky sessions, also known as [client affinity](/iis/extensions/configuring-application-request-routing-arr/http-load-balancing-using-application-request-routing#step-3---configure-client-affinity), is not required, because clients are immediately redirected to the Azure SignalR Service when they connect.</span></span>
* <span data-ttu-id="909fb-148">SignalR應用程式可以根據傳送的訊息數目進行擴充，而 Azure 服務則會 SignalR 調整以處理任何數量的連線。</span><span class="sxs-lookup"><span data-stu-id="909fb-148">A SignalR app can scale out based on the number of messages sent, while the Azure SignalR Service scales to handle any number of connections.</span></span> <span data-ttu-id="909fb-149">例如，可能有數千個用戶端，但如果每秒只傳送幾個訊息，則 SignalR 應用程式不需要向外延展至多部伺服器，只要處理連線本身即可。</span><span class="sxs-lookup"><span data-stu-id="909fb-149">For example, there could be thousands of clients, but if only a few messages per second are sent, the SignalR app won't need to scale out to multiple servers just to handle the connections themselves.</span></span>
* <span data-ttu-id="909fb-150">SignalR應用程式不會使用與 web 應用程式明顯更多的連接資源 SignalR 。</span><span class="sxs-lookup"><span data-stu-id="909fb-150">A SignalR app won't use significantly more connection resources than a web app without SignalR.</span></span>

<span data-ttu-id="909fb-151">基於這些理由，我們建議 azure SignalR 服務裝載 SignalR 于 azure 上的所有 ASP.NET Core 應用程式，包括 App Service、vm 和容器。</span><span class="sxs-lookup"><span data-stu-id="909fb-151">For these reasons, we recommend the Azure SignalR Service for all ASP.NET Core SignalR apps hosted on Azure, including App Service, VMs, and containers.</span></span>

<span data-ttu-id="909fb-152">如需詳細資訊，請參閱 [Azure SignalR 服務檔](/azure/azure-signalr/signalr-overview)。</span><span class="sxs-lookup"><span data-stu-id="909fb-152">For more information see the [Azure SignalR Service documentation](/azure/azure-signalr/signalr-overview).</span></span>
...
<span data-ttu-id="909fb-154">[Redis](https://redis.io/) 是記憶體中的索引鍵/值存放區，可支援具有發佈/訂閱模型的訊息系統。</span><span class="sxs-lookup"><span data-stu-id="909fb-154">[Redis](https://redis.io/) is an in-memory key-value store that supports a messaging system with a publish/subscribe model.</span></span> <span data-ttu-id="909fb-155">SignalRRedis 背板使用 pub/sub 功能將訊息轉送至其他伺服器。</span><span class="sxs-lookup"><span data-stu-id="909fb-155">The SignalR Redis backplane uses the pub/sub feature to forward messages to other servers.</span></span> <span data-ttu-id="909fb-156">當用戶端建立連接時，會將連接資訊傳遞給後端。</span><span class="sxs-lookup"><span data-stu-id="909fb-156">When a client makes a connection, the connection information is passed to the backplane.</span></span> <span data-ttu-id="909fb-157">當伺服器想要將訊息傳送給所有用戶端時，它會傳送至後端。</span><span class="sxs-lookup"><span data-stu-id="909fb-157">When a server wants to send a message to all clients, it sends to the backplane.</span></span> <span data-ttu-id="909fb-158">後擋板知道所有連線的用戶端，以及它們所在的伺服器。</span><span class="sxs-lookup"><span data-stu-id="909fb-158">The backplane knows all connected clients and which servers they're on.</span></span> <span data-ttu-id="909fb-159">它會透過各自的伺服器將訊息傳送給所有用戶端。</span><span class="sxs-lookup"><span data-stu-id="909fb-159">It sends the message to all clients via their respective servers.</span></span> <span data-ttu-id="909fb-160">下圖說明此程式：</span><span class="sxs-lookup"><span data-stu-id="909fb-160">This process is illustrated in the following diagram:</span></span>

![Redis 後擋板，從一部伺服器傳送到所有用戶端的訊息](scale/_static/redis-backplane.png)

<span data-ttu-id="909fb-162">針對裝載在您自己的基礎結構上的應用程式，Redis 背板是建議的向外延展方法。</span><span class="sxs-lookup"><span data-stu-id="909fb-162">The Redis backplane is the recommended scale-out approach for apps hosted on your own infrastructure.</span></span> <span data-ttu-id="909fb-163">如果您的資料中心與 Azure 資料中心之間有明顯的連線延遲，則 SignalR 對於低延遲或高輸送量需求的內部部署應用程式，Azure 服務可能不是實際的選項。</span><span class="sxs-lookup"><span data-stu-id="909fb-163">If there is significant connection latency between your data center and an Azure data center, Azure SignalR Service may not be a practical option for on-premises apps with low latency or high throughput requirements.</span></span>

<span data-ttu-id="909fb-164">稍早所述的 Azure SignalR 服務優點是 Redis 背板的缺點：</span><span class="sxs-lookup"><span data-stu-id="909fb-164">The Azure SignalR Service advantages noted earlier are disadvantages for the Redis backplane:</span></span>
...
* <span data-ttu-id="909fb-169">SignalR應用程式必須根據用戶端的數目進行相應放大，即使傳送的訊息數很少。</span><span class="sxs-lookup"><span data-stu-id="909fb-169">A SignalR app must scale out based on number of clients even if few messages are being sent.</span></span>
* <span data-ttu-id="909fb-170">SignalR應用程式在沒有的情況下，會使用比 web 應用程式更多的連接資源 SignalR 。</span><span class="sxs-lookup"><span data-stu-id="909fb-170">A SignalR app uses significantly more connection resources than a web app without SignalR.</span></span>
...
<span data-ttu-id="909fb-172">Windows 10 和 Windows 8. x 是用戶端作業系統。</span><span class="sxs-lookup"><span data-stu-id="909fb-172">Windows 10 and Windows 8.x are client operating systems.</span></span> <span data-ttu-id="909fb-173">用戶端作業系統上的 IIS 有10個並行連接的限制。</span><span class="sxs-lookup"><span data-stu-id="909fb-173">IIS on client operating systems has a limit of 10 concurrent connections.</span></span> <span data-ttu-id="909fb-174">SignalR的連接包括：</span><span class="sxs-lookup"><span data-stu-id="909fb-174">SignalR's connections are:</span></span>
...
<span data-ttu-id="909fb-182">以下包含啟用 Websocket、ServerSentEvents 和 LongPolling 的最低必要設定 SignalR ：</span><span class="sxs-lookup"><span data-stu-id="909fb-182">The following contains the minimum required settings to enable WebSockets, ServerSentEvents, and LongPolling for SignalR:</span></span>
...
<span data-ttu-id="909fb-183">使用多個後端伺服器時，必須新增「粘滯話」以防止連線 SignalR 在連接時切換伺服器。</span><span class="sxs-lookup"><span data-stu-id="909fb-183">When multiple backend servers are used, sticky sessions must be added to prevent SignalR connections from switching servers when connecting.</span></span> <span data-ttu-id="909fb-184">有多種方式可以在 Nginx 中新增多個會話。</span><span class="sxs-lookup"><span data-stu-id="909fb-184">There are multiple ways to add sticky sessions in Nginx.</span></span> <span data-ttu-id="909fb-185">下列兩種方法會根據您可用的內容顯示在下方。</span><span class="sxs-lookup"><span data-stu-id="909fb-185">Two approaches are shown below depending on what you have available.</span></span>
...
<span data-ttu-id="909fb-189">使用 [Nginx Plus](https://www.nginx.com/products/nginx)，請使用將 `sticky` 新增 cookie 至要求，並將使用者的要求釘選到伺服器：</span><span class="sxs-lookup"><span data-stu-id="909fb-189">With [Nginx Plus](https://www.nginx.com/products/nginx), use `sticky` to add a cookie to requests and pin the user's requests to a server:</span></span>
...
## <a name="third-party-no-locsignalr-backplane-providers"></a><span data-ttu-id="909fb-194">協力廠商 SignalR 背板提供者</span><span class="sxs-lookup"><span data-stu-id="909fb-194">Third-party SignalR backplane providers</span></span>
...
* <span data-ttu-id="909fb-196">[新奧爾良](https://github.com/OrleansContrib/SignalR.Orleans)</span><span class="sxs-lookup"><span data-stu-id="909fb-196">[Orleans](https://github.com/OrleansContrib/SignalR.Orleans)</span></span>
...
* [<span data-ttu-id="909fb-199">Azure SignalR 服務檔</span><span class="sxs-lookup"><span data-stu-id="909fb-199">Azure SignalR Service documentation</span></span>](/azure/azure-signalr/signalr-overview)
