...
<span data-ttu-id="92766-105">在本節中將驗證邏輯新增至 `Movie` 模型。</span><span class="sxs-lookup"><span data-stu-id="92766-105">In this section, validation logic is added to the `Movie` model.</span></span> <span data-ttu-id="92766-106">使用者建立或編輯電影時，就會強制執行驗證規則。</span><span class="sxs-lookup"><span data-stu-id="92766-106">The validation rules are enforced any time a user creates or edits a movie.</span></span>
...
## <a name="add-validation-rules-to-the-movie-model"></a><span data-ttu-id="92766-116">將驗證規則新增至電影模型</span><span class="sxs-lookup"><span data-stu-id="92766-116">Add validation rules to the movie model</span></span>

<span data-ttu-id="92766-117">`DataAnnotations`命名空間提供：</span><span class="sxs-lookup"><span data-stu-id="92766-117">The `DataAnnotations` namespace provides:</span></span>

* <span data-ttu-id="92766-118">一組內建的驗證屬性，會以宣告方式套用至類別或屬性。</span><span class="sxs-lookup"><span data-stu-id="92766-118">A set of built-in validation attributes that are applied declaratively to a class or property.</span></span>
* <span data-ttu-id="92766-119">格式化屬性 `[DataType]` ，例如格式化的說明，而且不提供任何驗證。</span><span class="sxs-lookup"><span data-stu-id="92766-119">Formatting attributes like `[DataType]` that help with formatting and don't provide any validation.</span></span>
...
* <span data-ttu-id="92766-122">`[Required]` 和 `[MinimumLength]` 屬性 (attribute) 指出屬性 (property) 必須具有值。</span><span class="sxs-lookup"><span data-stu-id="92766-122">The `[Required]` and `[MinimumLength]` attributes indicate that a property must have a value.</span></span> <span data-ttu-id="92766-123">無需防止使用者輸入空白字元來滿足這種驗證。</span><span class="sxs-lookup"><span data-stu-id="92766-123">Nothing prevents a user from entering white space to satisfy this validation.</span></span>
* <span data-ttu-id="92766-124">`[RegularExpression]` 屬性則用來限制可輸入的字元。</span><span class="sxs-lookup"><span data-stu-id="92766-124">The `[RegularExpression]` attribute is used to limit what characters can be input.</span></span> <span data-ttu-id="92766-125">在上述程式碼中， `Genre` ：</span><span class="sxs-lookup"><span data-stu-id="92766-125">In the preceding code, `Genre`:</span></span>

  * <span data-ttu-id="92766-126">必須指使用字母。</span><span class="sxs-lookup"><span data-stu-id="92766-126">Must only use letters.</span></span>
  * <span data-ttu-id="92766-127">第一個字母必須是大寫。</span><span class="sxs-lookup"><span data-stu-id="92766-127">The first letter is required to be uppercase.</span></span> <span data-ttu-id="92766-128">不允許使用空格、數字和特殊字元。</span><span class="sxs-lookup"><span data-stu-id="92766-128">White space, numbers, and special characters are not allowed.</span></span>

* <span data-ttu-id="92766-129">`RegularExpression` `Rating` ：</span><span class="sxs-lookup"><span data-stu-id="92766-129">The `RegularExpression` `Rating`:</span></span>

  * <span data-ttu-id="92766-130">第一個字元必須為大寫字母。</span><span class="sxs-lookup"><span data-stu-id="92766-130">Requires that the first character be an uppercase letter.</span></span>
  * <span data-ttu-id="92766-131">允許後續空格中的特殊字元和數位。</span><span class="sxs-lookup"><span data-stu-id="92766-131">Allows special characters and numbers in subsequent spaces.</span></span> <span data-ttu-id="92766-132">"PG-13" 適用于評等，但失敗 `Genre` 。</span><span class="sxs-lookup"><span data-stu-id="92766-132">"PG-13" is valid for a rating, but fails for a `Genre`.</span></span>

* <span data-ttu-id="92766-133">`[Range]` 屬性會將值限制在指定的範圍內。</span><span class="sxs-lookup"><span data-stu-id="92766-133">The `[Range]` attribute constrains a value to within a specified range.</span></span>
* <span data-ttu-id="92766-134">`[StringLength]`屬性可以設定字串屬性的最大長度，並選擇性地設定其最小長度。</span><span class="sxs-lookup"><span data-stu-id="92766-134">The `[StringLength]` attribute can set a maximum length of a string property, and optionally its minimum length.</span></span>
* <span data-ttu-id="92766-135">實數值型別（例如，、、、） `decimal` `int` 本質上 `float` `DateTime` 是必要的，而且不需要 `[Required]` 屬性。</span><span class="sxs-lookup"><span data-stu-id="92766-135">Value types, such as `decimal`, `int`, `float`, `DateTime`, are inherently required and don't need the `[Required]` attribute.</span></span>

<span data-ttu-id="92766-136">上述驗證規則是用於示範，不是生產系統的最佳做法。</span><span class="sxs-lookup"><span data-stu-id="92766-136">The preceding validation rules are used for demonstration, they are not optimal for a production system.</span></span> <span data-ttu-id="92766-137">例如，上述程式可避免輸入只有兩個字元的電影，而不允許在中使用特殊字元 `Genre` 。</span><span class="sxs-lookup"><span data-stu-id="92766-137">For example, the preceding prevents entering a movie with only two chars and doesn't allow special characters in `Genre`.</span></span>

<span data-ttu-id="92766-138">具有 ASP.NET Core 自動強制執行的驗證規則有助於：</span><span class="sxs-lookup"><span data-stu-id="92766-138">Having validation rules automatically enforced by ASP.NET Core helps:</span></span>
...
### <a name="server-side-validation"></a><span data-ttu-id="92766-160">伺服器端驗證</span><span class="sxs-lookup"><span data-stu-id="92766-160">Server-side validation</span></span>

<span data-ttu-id="92766-161">在瀏覽器中停用 JavaScript 時，提交含有錯誤的表單將發佈到伺服器。</span><span class="sxs-lookup"><span data-stu-id="92766-161">When JavaScript is disabled in the browser, submitting the form with errors will post to the server.</span></span>
...
## <a name="use-datatype-attributes"></a><span data-ttu-id="92766-183">使用 DataType 屬性</span><span class="sxs-lookup"><span data-stu-id="92766-183">Use DataType Attributes</span></span>
...
* <span data-ttu-id="92766-188">查看引擎格式化資料的提示。</span><span class="sxs-lookup"><span data-stu-id="92766-188">Hints for the view engine to format the data.</span></span>
* <span data-ttu-id="92766-189">提供屬性，例如 `<a>` URL 和 `<a href="mailto:EmailAddress.com">` 電子郵件。</span><span class="sxs-lookup"><span data-stu-id="92766-189">Supplies attributes such as `<a>` for URL's and `<a href="mailto:EmailAddress.com">` for email.</span></span>

<span data-ttu-id="92766-190">使用 `[RegularExpression]` 屬性來驗證資料的格式。</span><span class="sxs-lookup"><span data-stu-id="92766-190">Use the `[RegularExpression]` attribute to validate the format of the data.</span></span> <span data-ttu-id="92766-191">`[DataType]` 屬性可用於指定比資料庫內建類型更特定的資料類型。</span><span class="sxs-lookup"><span data-stu-id="92766-191">The `[DataType]` attribute is used to specify a data type that's more specific than the database intrinsic type.</span></span> <span data-ttu-id="92766-192">`[DataType]` 屬性不是驗證屬性。</span><span class="sxs-lookup"><span data-stu-id="92766-192">`[DataType]` attributes aren't validation attributes.</span></span> <span data-ttu-id="92766-193">在範例應用程式中，只會顯示日期，而不含時間。</span><span class="sxs-lookup"><span data-stu-id="92766-193">In the sample application, only the date is displayed, without time.</span></span>

<span data-ttu-id="92766-194">`DataType`列舉提供許多資料類型，例如、、 `Date` `Time` `PhoneNumber` 、、等等 `Currency` `EmailAddress` 。</span><span class="sxs-lookup"><span data-stu-id="92766-194">The `DataType` enumeration provides many data types, such as `Date`, `Time`, `PhoneNumber`, `Currency`, `EmailAddress`, and more.</span></span> 

<span data-ttu-id="92766-195">`[DataType]`屬性：</span><span class="sxs-lookup"><span data-stu-id="92766-195">The `[DataType]` attributes:</span></span>

* <span data-ttu-id="92766-196">可以讓應用程式自動提供型別特有的功能。</span><span class="sxs-lookup"><span data-stu-id="92766-196">Can enable the application to automatically provide type-specific features.</span></span> <span data-ttu-id="92766-197">例如，可針對 `DataType.EmailAddress` 建立 `mailto:` 連結。</span><span class="sxs-lookup"><span data-stu-id="92766-197">For example, a `mailto:` link can be created for `DataType.EmailAddress`.</span></span>
* <span data-ttu-id="92766-198">可以 `DataType.Date` 在支援 HTML5 的瀏覽器中提供日期選擇器。</span><span class="sxs-lookup"><span data-stu-id="92766-198">Can provide a date selector `DataType.Date` in browsers that support HTML5.</span></span>
* <span data-ttu-id="92766-199">發出 HTML 5 `data-` ，發音為「資料虛線」，這是 html 5 瀏覽器使用的屬性。</span><span class="sxs-lookup"><span data-stu-id="92766-199">Emit HTML 5 `data-`, pronounced "data dash", attributes that HTML 5 browsers consume.</span></span>
* <span data-ttu-id="92766-200">請勿 **提供任何** 驗證。</span><span class="sxs-lookup"><span data-stu-id="92766-200">Do **not** provide any validation.</span></span>

<span data-ttu-id="92766-201">`DataType.Date` 未指定顯示日期的格式。</span><span class="sxs-lookup"><span data-stu-id="92766-201">`DataType.Date` doesn't specify the format of the date that's displayed.</span></span> <span data-ttu-id="92766-202">根據預設，將依據以伺服器 `CultureInfo` 為基礎的預設格式顯示資料欄位。</span><span class="sxs-lookup"><span data-stu-id="92766-202">By default, the data field is displayed according to the default formats based on the server's `CultureInfo`.</span></span>

<span data-ttu-id="92766-203">`[Column(TypeName = "decimal(18, 2)")]` 資料註解為必要項，因此 Entity Framework Core 可將 `Price` 正確對應到資料庫中的貨幣。</span><span class="sxs-lookup"><span data-stu-id="92766-203">The `[Column(TypeName = "decimal(18, 2)")]` data annotation is required so Entity Framework Core can correctly map `Price` to currency in the database.</span></span> <span data-ttu-id="92766-204">如需詳細資訊，請參閱[資料類型](/ef/core/modeling/relational/data-types)。</span><span class="sxs-lookup"><span data-stu-id="92766-204">For more information, see [Data Types](/ef/core/modeling/relational/data-types).</span></span>

<span data-ttu-id="92766-205">`[DisplayFormat]` 屬性用來明確指定日期格式：</span><span class="sxs-lookup"><span data-stu-id="92766-205">The `[DisplayFormat]` attribute is used to explicitly specify the date format:</span></span>
...
<span data-ttu-id="92766-206">`ApplyFormatInEditMode`設定會指定在顯示值以供編輯時，將套用格式設定。</span><span class="sxs-lookup"><span data-stu-id="92766-206">The `ApplyFormatInEditMode` setting specifies that the formatting will be applied when the value is displayed for editing.</span></span> <span data-ttu-id="92766-207">某些欄位可能不需要該行為。</span><span class="sxs-lookup"><span data-stu-id="92766-207">That behavior may not be wanted for some fields.</span></span> <span data-ttu-id="92766-208">例如，在貨幣值中，在編輯 UI 中通常不需要貨幣符號。</span><span class="sxs-lookup"><span data-stu-id="92766-208">For example, in currency values, the currency symbol is usually not wanted in the edit UI.</span></span>

<span data-ttu-id="92766-209">`[DisplayFormat]` 屬性可以單獨使用，但通常最好是使用 `[DataType]` 屬性。</span><span class="sxs-lookup"><span data-stu-id="92766-209">The `[DisplayFormat]` attribute can be used by itself, but it's generally a good idea to use the `[DataType]` attribute.</span></span> <span data-ttu-id="92766-210">`[DataType]` 屬性會將資料的語意以其在螢幕上呈現方式的相反方式傳遞。</span><span class="sxs-lookup"><span data-stu-id="92766-210">The `[DataType]` attribute conveys the semantics of the data as opposed to how to render it on a screen.</span></span> <span data-ttu-id="92766-211">`[DataType]`屬性提供下列無法使用的優點 `[DisplayFormat]` ：</span><span class="sxs-lookup"><span data-stu-id="92766-211">The `[DataType]` attribute provides the following benefits that aren't available with `[DisplayFormat]`:</span></span>

* <span data-ttu-id="92766-212">瀏覽器可以啟用 HTML5 功能，例如顯示行事曆控制項、地區設定適當的貨幣符號、電子郵件連結等等。</span><span class="sxs-lookup"><span data-stu-id="92766-212">The browser can enable HTML5 features, for example to show a calendar control, the locale-appropriate currency symbol, email links, etc.</span></span>
* <span data-ttu-id="92766-213">根據預設，瀏覽器會根據其地區設定，使用正確的格式來呈現資料。</span><span class="sxs-lookup"><span data-stu-id="92766-213">By default, the browser renders data using the correct format based on its locale.</span></span>
* <span data-ttu-id="92766-214">`[DataType]` 屬性可以啟用 ASP.NET Core 架構，來選擇用於呈現資料的正確欄位範本。</span><span class="sxs-lookup"><span data-stu-id="92766-214">The `[DataType]` attribute can enable the ASP.NET Core framework to choose the right field template to render the data.</span></span> <span data-ttu-id="92766-215">`DisplayFormat`如果本身使用，則會使用字串範本。</span><span class="sxs-lookup"><span data-stu-id="92766-215">The `DisplayFormat`, if used by itself, uses the string template.</span></span>

<span data-ttu-id="92766-216">**注意：** jQuery 驗證不適用於 `[Range]` 屬性和 `DateTime` 。</span><span class="sxs-lookup"><span data-stu-id="92766-216">**Note:** jQuery validation doesn't work with the `[Range]` attribute and `DateTime`.</span></span> <span data-ttu-id="92766-217">例如，下列程式碼一律會顯示用戶端驗證錯誤，即使當日期位在指定範圍中也一樣：</span><span class="sxs-lookup"><span data-stu-id="92766-217">For example, the following code will always display a client-side validation error, even when the date is in the specified range:</span></span>

```csharp
[Range(typeof(DateTime), "1/1/1966", "1/1/2020")]
   ```

<span data-ttu-id="92766-218">最佳做法是避免在模型中編譯硬性日期，因此 `[Range]` `DateTime` 不建議使用屬性。</span><span class="sxs-lookup"><span data-stu-id="92766-218">It's a best practice to avoid compiling hard dates in models, so using the `[Range]` attribute and `DateTime` is discouraged.</span></span> <span data-ttu-id="92766-219">使用日期範圍的設定以及其他受頻繁 [變更的值](xref:fundamentals/configuration/index) ，而不是在程式碼中指定。</span><span class="sxs-lookup"><span data-stu-id="92766-219">Use [Configuration](xref:fundamentals/configuration/index) for date ranges and other values that are subject to frequent change rather than specifying it in code.</span></span>
...
# <a name="visual-studio"></a>[<span data-ttu-id="92766-227">Visual Studio</span><span class="sxs-lookup"><span data-stu-id="92766-227">Visual Studio</span></span>](#tab/visual-studio)

<span data-ttu-id="92766-228">`Movie` 資料表目前具有下列結構描述：</span><span class="sxs-lookup"><span data-stu-id="92766-228">The `Movie` table currently has the following schema:</span></span>
...
<span data-ttu-id="92766-229">上述結構描述變更不會造成 EF 擲回例外狀況。</span><span class="sxs-lookup"><span data-stu-id="92766-229">The preceding schema changes don't cause EF to throw an exception.</span></span> <span data-ttu-id="92766-230">不過，請建立移轉，讓結構描述與模型一致。</span><span class="sxs-lookup"><span data-stu-id="92766-230">However, create a migration so the schema is consistent with the model.</span></span>

<span data-ttu-id="92766-231"> 從 [工具] 功能表中，選取 [NuGet 套件管理員] > [套件管理員主控台]。</span><span class="sxs-lookup"><span data-stu-id="92766-231">From the **Tools** menu, select **NuGet Package Manager > Package Manager Console**.</span></span>
<span data-ttu-id="92766-232">在 PMC 中，輸入下列命令：</span><span class="sxs-lookup"><span data-stu-id="92766-232">In the PMC, enter the following commands:</span></span>
...
# <a name="visual-studio-code--visual-studio-for-mac"></a>[<span data-ttu-id="92766-236">Visual Studio Code / Visual Studio for Mac</span><span class="sxs-lookup"><span data-stu-id="92766-236">Visual Studio Code / Visual Studio for Mac</span></span>](#tab/visual-studio-code+visual-studio-mac)

<span data-ttu-id="92766-237">SQLite 不需要移轉。</span><span class="sxs-lookup"><span data-stu-id="92766-237">Migrations are not required for SQLite.</span></span>

---

### <a name="publish-to-azure"></a><span data-ttu-id="92766-238">發佈至 Azure</span><span class="sxs-lookup"><span data-stu-id="92766-238">Publish to Azure</span></span>
...
> [<span data-ttu-id="92766-243">上一步：新增欄位</span><span class="sxs-lookup"><span data-stu-id="92766-243">Previous: Add a new field</span></span>](xref:tutorials/razor-pages/new-field)
