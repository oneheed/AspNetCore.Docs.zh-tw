...
上述命令會新增 [Microsoft typescript](https://www.nuget.org/packages/Microsoft.TypeScript.MSBuild/) 封裝，在專案中啟用 TypeScript 編譯。
...
1. `scripts`以下列程式碼取代檔案 *上package.js* 的屬性：
    ...
    * `build`：將用戶端資源組合于開發模式，並監看檔案變更。 檔案監看員會導致套件組合在每次專案檔變更時重新產生。 `mode` 選項會停用生產環境最佳化，例如樹狀結構搖晃和縮製。 `build` 僅用於開發。
...
1. 使用下列程式碼，在專案根目錄中建立名為 *webpack.config.js* 的檔案：
    ...
    * `resolve.extensions`陣列包含要匯入用戶端 JavaScript 的 *.js。* SignalR
...
1. 建立具有下列 CSS 的 *src/css/main .css* ：
...
1. 使用下列程式碼建立 *src/index. ts* ：
    ...
    * `keyup`：當使用者在文字方塊中輸入時，就會引發此事件 `tbMessage` 。 當使用者按下 **Enter** 鍵時，即會呼叫 `send` 函式。
    * `click`：當使用者按一下 [傳送]  按鈕時，就會引發此事件。 系統會呼叫 `send` 函式。

## <a name="configure-the-app"></a>設定應用程式

1. 在中 `Startup.Configure` ，新增對 [UseDefaultFiles](/dotnet/api/microsoft.aspnetcore.builder.defaultfilesextensions.usedefaultfiles#Microsoft_AspNetCore_Builder_DefaultFilesExtensions_UseDefaultFiles_Microsoft_AspNetCore_Builder_IApplicationBuilder_) 和 [>usestaticfiles](/dotnet/api/microsoft.aspnetcore.builder.staticfileextensions.usestaticfiles#Microsoft_AspNetCore_Builder_StaticFileExtensions_UseStaticFiles_Microsoft_AspNetCore_Builder_IApplicationBuilder_)的呼叫。
...
1. 在專案根目錄 *SignalR WebPack* 中建立名為「 *中樞* 」的新目錄，以儲存 SignalR 中樞。

1. 使用下列程式碼建立中樞 *Hubs/ChatHub.cs* ：

    [!code-csharp[ChatHub](signalr-typescript-webpack/sample/3.x/snippets/ChatHub.cs?name=snippet_ChatHubStubClass)]

1. `using`在 *Startup.cs* 檔案的頂端加入下列語句，以解析 `ChatHub` 參考：

    [!code-csharp[Startup](signalr-typescript-webpack/sample/3.x/Startup.cs?name=snippet_HubsNamespace)]

## <a name="enable-client-and-server-communication"></a>啟用用戶端與伺服器的通訊

應用程式目前顯示傳送訊息的基本表單，但還無法正常運作。 伺服器正在接聽特定的路由，但對於已傳送的訊息不會進行任何處理。
...
1. 在專案根目錄中執行下列命令：
     ...
     * Node.js 的 TypeScript 類型定義，可進行 Node.js 類型的編譯時間檢查。
...
1. 將醒目提示的程式碼新增至 *src/index.ts* 檔案：
    ...
    SignalR 啟用用戶端與伺服器之間的訊息交換。 每個訊息都有特定的名稱。 例如，具有名稱的訊息 `messageReceived` 可以執行邏輯，負責在訊息區域中顯示新訊息。 接聽特定的訊息可透過 `on` 函式完成。 您可以聽取任何數量的訊息名稱。 也可將參數傳遞給訊息，例如作者的名稱和已接收訊息的內容。 用戶端收到訊息之後，就會在其 `innerHTML` 屬性中使用作者的名稱和訊息內容建立新的 `div` 元素。 它會新增至顯示訊息的主要 `div` 元素。
...
1. 將 `NewMessage` 方法新增至 `ChatHub` 類別：
    ...
    在此範例中，TypeScript 用戶端會傳送一則識別為 `newMessage` 的訊息。 C# `NewMessage` 方法預期有用戶端所傳送的資料。 在[用戶端](/dotnet/api/microsoft.aspnetcore.signalr.ihubclients-1.all)上進行[SendAsync](/dotnet/api/microsoft.aspnetcore.signalr.clientproxyextensions.sendasync)呼叫。 接收的訊息就會傳送到連線至中樞的所有用戶端。
...
1. 選取 **Debug**  >  [在不進行偵錯工具的 **情況下啟動** ]，即可在瀏覽器中啟動應用程式，而不需附加 隨即會在 `http://localhost:<port_number>` 處提供 *wwwroot/index.html* 檔案。

   如果您收到編譯錯誤，請嘗試關閉並重新開啟方案。 
...
1. 使用 [檔案]  功能表選項，然後選擇 [ASP.NET Core Web 應用程式]  範本。
1. 將專案命名為 *SignalR WebPack* ，然後選取 [ **建立** ]。
...
1. 在專案根目錄中建立名為 *Hubs* 的新目錄。 其目的是要儲存在 SignalR 下一個步驟中建立的中樞。
...
1. 將醒目提示的程式碼新增至 *src/index.ts* 檔案：
    ...
    SignalR 啟用用戶端與伺服器之間的訊息交換。 每個訊息都有特定的名稱。 例如，具有名稱的訊息 `messageReceived` 可以執行邏輯，負責在訊息區域中顯示新訊息。 接聽特定的訊息可透過 `on` 函式完成。 您可以接聽任意數目的訊息名稱。 也可將參數傳遞給訊息，例如作者的名稱和已接收訊息的內容。 用戶端收到訊息之後，就會在其 `innerHTML` 屬性中使用作者的名稱和訊息內容建立新的 `div` 元素。 新的訊息會新增至 `div` 顯示訊息的主要元素。
