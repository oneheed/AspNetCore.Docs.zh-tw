...
[查看或下載範例程式碼](https://github.com/dotnet/AspNetCore.Docs/tree/main/aspnetcore/web-api/action-return-types/samples) ([如何下載](xref:index#how-to-download-a-sample)) 
...
* 當產品存在時，會傳回200狀態碼和對應的 `Product` 物件。 在 ASP.NET 核心2.1 之前， `return product;` 行必須是 `return Ok(product);` 。
...
* <xref:Microsoft.AspNetCore.Mvc.ControllerBase.BadRequest*>ASP.NET Core 執行時間會傳回400狀態碼 () ：
